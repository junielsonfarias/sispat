...
class SystemTester {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.recordTest(
            `table_${table}`,
            true,
            `${data.record_count} registros encontrados`
          );
        } else {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          log.success(
            `${endpoint.name}: ${Array.isArray(data) ? data.length : 'OK'}`
          );
          this.recordTest(
    ...
    if (this.testData.local) {
      try {
        ...
        const response = await fetch(
          `${BASE_URL}/locals/${this.testData.local.id}`,
          {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
  ...
  async testDeleteOperations() {
    ...
    if (this.testData.patrimonio) {
      try {
        const response = await fetch(
          `${BASE_URL}/patrimonios/${this.testData.patrimonio.id}`,
          {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${this.token}` },
          }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.errors && data.errors.length > 0) {
          log.warning(
            `Encontrados ${data.errors.length} erros de integridade:`
          );
  ...
  async cleanupTestData() {
    ...
    if (this.testData.municipality) {
      try {
        await fetch(
          `${BASE_URL}/municipalities/${this.testData.municipality.id}`,
          {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${this.token}` },
          }
