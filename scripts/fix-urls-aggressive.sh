...
# =============================================================================
# SCRIPT AGRESSIVO - CORREÇÃO FORÇADA DE URLs
# Força a substituição de todas as URLs HTTPS por HTTP
# =============================================================================
...
╔══════════════════════════════════════════════════════════════╗
║                                                              ║
║        🔥 CORREÇÃO AGRESSIVA DE URLs HARDCODED              ║
║                                                              ║
║              Força substituição de HTTPS por HTTP           ║
║              em TODOS os arquivos de build                  ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
EOF
echo -e "${NC}"
...
# Função para correção agressiva de URLs
fix_urls_aggressive() {
    log_header "Iniciando correção agressiva de URLs..."
    ...
    log_info "Procurando TODOS os arquivos com URLs HTTPS..."
    
    # Lista de padrões a serem corrigidos de forma agressiva
    local patterns=(
        "https://$DOMAIN"
    ...
    for pattern in "${patterns[@]}"; do
        ...
        find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" -o -name "*.json" \) -exec grep -l "$pattern" {} \; | while read -r file; do
            if [ -f "$file" ]; then
    ...
    # Correção adicional: substituir QUALQUER HTTPS por HTTP no domínio
    log_info "Substituindo QUALQUER HTTPS por HTTP no domínio..."
    find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" -o -name "*.json" \) -exec sed -i "s|https://$DOMAIN|http://$DOMAIN|g" {} \;
    
    # Correção adicional: substituir localhost por domínio
    log_info "Substituindo localhost por domínio..."
    ...
    find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" -o -name "*.json" \) -exec sed -i "s|localhost:3000|$DOMAIN|g" {} \;
    
    log_success "Correção agressiva concluída!"
}
...
verify_fixes() {
    ...
    local https_count=$(find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" -o -name "*.json" \) -exec grep -l "https://" {} \; | wc -l)
    local localhost_count=$(find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" -o -name "*.json" \) -exec grep -l "localhost:" {} \; | wc -l)
    ...
    if [ "$https_count" -eq 0 ] && [ "$localhost_count" -eq 0 ]; then
        ...
        if [ "$https_count" -gt 0 ]; then
            ...
            find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" -o -name "*.json" \) -exec grep -l "https://" {} \;
        fi
        
        if [ "$localhost_count" -gt 0 ]; then
            log_info "Arquivos com localhost:"
            find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" -o -name "*.json" \) -exec grep -l "localhost:" {} \;
        fi
    fi
}
...
show_final_info() {
    log_header "Correção Agressiva Concluída!"
    
    echo -e "\n${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║                                                              ║${NC}"
    echo -e "${GREEN}║           ✅ CORREÇÃO AGRESSIVA DE URLs CONCLUÍDA!          ║${NC}"
    echo -e "${GREEN}║                                                              ║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
    ...
    echo -e "✅ URLs HTTPS corrigidas para HTTP (correção agressiva)"
...
# Função principal
main() {
    log_header "Iniciando correção agressiva de URLs hardcoded..."
