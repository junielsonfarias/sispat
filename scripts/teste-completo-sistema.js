...
class TesteCompletoSistema {
  ...
  async login() {
    ...
    if (result.success && result.data?.token) {
      ...
      await this.logger.log(`✅ Login realizado com sucesso! Token: ${this.token.substring(0, 20)}...`)
      await this.logger.log(`👤 Usuário: ${result.data.user.name} (ID: ${this.userId})`)
      await this.logger.log(`🏛️ Município: ${result.data.user.municipality_name} (ID: ${this.municipalityId})`)
      
      return true
    } else {
      this.stats.login.sucesso = false
      await this.logger.log(`❌ Falha no login: ${result.error || result.data?.error || 'Erro desconhecido'}`, 'ERROR')
      return false
    }
  }

  gerarDadosBem(index) {
    const tipos = ['Equipamento de Informática', 'Móveis e Utensílios', 'Veículos', 'Imóveis', 'Equipamentos de Escritório']
    const marcas = ['Dell', 'HP', 'Samsung', 'LG', 'Apple', 'Lenovo', 'Asus', 'Acer']
    ...
    return {
      ...
      data_aquisicao: new Date(2024, 0, 1 + (index % 365)).toISOString().split('T')[0],
  ...
  async criarBem(dadosBem) {
    const result = await this.makeRequest(`${CONFIG.BASE_URL}/api/patrimonios`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  async criarBens() {
    await this.logger.log(`🏗️ Iniciando criação de ${CONFIG.TEST_DATA.totalBens} bens patrimoniais...`)
    const startTime = Date.now()
    
    for (let i = 0; i < CONFIG.TEST_DATA.totalBens; i++) {
      const dadosBem = this.gerarDadosBem(i)
      
      await this.logger.log(`📝 Criando bem ${i + 1}/${CONFIG.TEST_DATA.totalBens}: ${dadosBem.numero_patrimonio}`)
      await this.logger.log(`🌐 Abra o navegador em: ${CONFIG.FRONTEND_URL}/bens-cadastrados para acompanhar`)
      
      const result = await this.criarBem(dadosBem)
      
      if (result.success) {
        this.stats.criacao.sucessos++
        await this.logger.log(`✅ Bem ${dadosBem.numero_patrimonio} criado com sucesso (ID: ${result.id})`)
      } else {
        this.stats.criacao.falhas++
        await this.logger.log(`❌ Falha ao criar bem ${dadosBem.numero_patrimonio}: ${result.error}`, 'ERROR')
      }
      ...
      if (i < CONFIG.TEST_DATA.totalBens - 1) {
        await this.logger.log(`⏳ Aguardando ${CONFIG.TEST_DATA.delayBetweenRequests/1000} segundos...`)
    ...
    await this.logger.log(`📊 Criação concluída: ${this.stats.criacao.sucessos} sucessos, ${this.stats.criacao.falhas} falhas`)
  }
  ...
  async editarBem(bem) {
    ...
    const result = await this.makeRequest(`${CONFIG.BASE_URL}/api/patrimonios/${bem.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${this.token}`
  ...
  async editarBens() {
    await this.logger.log(`✏️ Iniciando edição de ${this.bensCriados.length} bens patrimoniais...`)
    const startTime = Date.now()
    
    for (let i = 0; i < this.bensCriados.length; i++) {
      const bem = this.bensCriados[i]
      
      await this.logger.log(`✏️ Editando bem ${i + 1}/${this.bensCriados.length}: ${bem.numero_patrimonio}`)
      await this.logger.log(`🌐 Abra o navegador em: ${CONFIG.FRONTEND_URL}/bens-cadastrados/editar/${bem.id} para acompanhar`)
      
      const result = await this.editarBem(bem)
      
      if (result.success) {
        this.stats.edicao.sucessos++
        await this.logger.log(`✅ Bem ${bem.numero_patrimonio} editado com sucesso`)
      } else {
        this.stats.edicao.falhas++
        await this.logger.log(`❌ Falha ao editar bem ${bem.numero_patrimonio}: ${result.error}`, 'ERROR')
      }
    ...
    await this.logger.log(`📊 Edição concluída: ${this.stats.edicao.sucessos} sucessos, ${this.stats.edicao.falhas} falhas`)
  }
  ...
  async excluirBens() {
    await this.logger.log(`🗑️ Iniciando exclusão de ${this.bensCriados.length} bens patrimoniais...`)
    const startTime = Date.now()
    
    for (let i = 0; i < this.bensCriados.length; i++) {
      const bem = this.bensCriados[i]
      
      await this.logger.log(`🗑️ Excluindo bem ${i + 1}/${this.bensCriados.length}: ${bem.numero_patrimonio}`)
      await this.logger.log(`🌐 Abra o navegador em: ${CONFIG.FRONTEND_URL}/bens-cadastrados para acompanhar`)
      ...
      if (result.success) {
        ...
        await this.logger.log(`✅ Bem ${bem.numero_patrimonio} excluído com sucesso`)
      } else {
        this.stats.exclusao.falhas++
        await this.logger.log(`❌ Falha ao excluir bem ${bem.numero_patrimonio}: ${result.error}`, 'ERROR')
      }
    ...
    await this.logger.log(`📊 Exclusão concluída: ${this.stats.exclusao.sucessos} sucessos, ${this.stats.exclusao.falhas} falhas`)
  }
  ...
  async verificarSincronizacao() {
    ...
    const result = await this.makeRequest(`${CONFIG.BASE_URL}/api/patrimonios?limit=1000`, {
      headers: {
        'Authorization': `Bearer ${this.token}`
      }
    })
    ...
    if (result.success && result.data?.items) {
      const bensEncontrados = result.data.items.filter(bem => 
        this.bensCriados.some(bemCriado => bemCriado.numero_patrimonio === bem.numero_patrimonio)
      )
      
      if (bensEncontrados.length === this.bensCriados.length) {
        this.stats.sincronizacao.sucessos++
        await this.logger.log(`✅ Sincronização OK: ${bensEncontrados.length} bens encontrados`)
      } else {
        this.stats.sincronizacao.falhas++
        await this.logger.log(`❌ Sincronização falhou: esperado ${this.bensCriados.length}, encontrado ${bensEncontrados.length}`, 'ERROR')
      }
    } else {
      this.stats.sincronizacao.falhas++
      await this.logger.log(`❌ Erro ao verificar sincronização: ${result.error}`, 'ERROR')
    }
  }
  ...
  async verificarLogsAtividade() {
    ...
    const result = await this.makeRequest(`${CONFIG.BASE_URL}/api/activity-logs?limit=100`, {
      headers: {
        'Authorization': `Bearer ${this.token}`
      }
    })
    ...
    if (result.success && result.data?.items) {
      ...
      await this.logger.log(`📋 Logs de atividade encontrados: ${logsPatrimonio.length} relacionados a patrimônios`)
      
      // Verificar se há logs para criação, edição e exclusão
      const logsCriacao = logsPatrimonio.filter(log => log.action === 'PATRIMONIO_CREATE')
      const logsEdicao = logsPatrimonio.filter(log => log.action === 'PATRIMONIO_UPDATE')
      const logsExclusao = logsPatrimonio.filter(log => log.action === 'PATRIMONIO_DELETE')
      ...
      await this.logger.log(`❌ Erro ao verificar logs de atividade: ${result.error}`, 'ERROR')
    }
  }
...
`
    ...
    try {
      ...
      await this.logger.log(`❌ Erro ao salvar relatório: ${error.message}`, 'ERROR')
    }
  }
  ...
  async executar() {
    ...
    await this.logger.log(`🌐 Para acompanhar no navegador, abra: ${CONFIG.FRONTEND_URL}`)
    ...
    try {
      ...
      await this.logger.log(`⏱️ Tempo total: ${this.stats.fim - this.stats.inicio}ms`)
      await this.logger.log(`📊 Total de operações: ${this.stats.criacao.total + this.stats.edicao.total + this.stats.exclusao.total}`)
      await this.logger.log(`📄 Relatório gerado: ${CONFIG.REPORT_FILE}`)
      
    } catch (error) {
      await this.logger.log(`❌ Erro durante o teste: ${error.message}`, 'ERROR')
