...
# =============================================================================
# SCRIPT DIRETO - CORREÇÃO DE URLs HARDCODED
# Corrige URLs hardcoded nos arquivos de build de forma direta e eficaz
# =============================================================================
...
╔══════════════════════════════════════════════════════════════╗
║                                                              ║
║        🔧 CORREÇÃO DIRETA DE URLs HARDCODED                 ║
║                                                              ║
║              Corrige URLs de forma direta e eficaz          ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
EOF
echo -e "${NC}"
...
# Função para corrigir URLs de forma direta
fix_urls_direct() {
    log_header "Corrigindo URLs de forma direta..."
    ...
    log_info "Procurando arquivos com URLs HTTPS..."
    
    # Encontrar todos os arquivos que contêm URLs HTTPS
    local files_with_https=$(find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" \) -exec grep -l "https://$DOMAIN" {} \;)
    
    if [ -z "$files_with_https" ]; then
        log_info "Nenhum arquivo com URLs HTTPS encontrado"
    else
        log_info "Arquivos com URLs HTTPS encontrados:"
        echo "$files_with_https"
        
        # Corrigir cada arquivo
        echo "$files_with_https" | while read -r file; do
            if [ -f "$file" ]; then
                log_info "Corrigindo: $file"
                sed -i "s|https://$DOMAIN|http://$DOMAIN|g" "$file"
            fi
        done
    fi
    
    # Corrigir URLs localhost
    log_info "Procurando arquivos com URLs localhost..."
    local files_with_localhost=$(find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" \) -exec grep -l "localhost:3001\|localhost:8080\|localhost:3000" {} \;)
    
    if [ -z "$files_with_localhost" ]; then
        log_info "Nenhum arquivo com URLs localhost encontrado"
    else
        log_info "Arquivos com URLs localhost encontrados:"
        echo "$files_with_localhost"
        
        # Corrigir cada arquivo
        echo "$files_with_localhost" | while read -r file; do
            if [ -f "$file" ]; then
                log_info "Corrigindo: $file"
                sed -i "s|https://localhost:3001|http://$DOMAIN|g" "$file"
                sed -i "s|http://localhost:3001|http://$DOMAIN|g" "$file"
                sed -i "s|https://localhost:8080|http://$DOMAIN|g" "$file"
                sed -i "s|http://localhost:8080|http://$DOMAIN|g" "$file"
                sed -i "s|https://localhost:3000|http://$DOMAIN|g" "$file"
                sed -i "s|http://localhost:3000|http://$DOMAIN|g" "$file"
            fi
        done
    fi
    
    log_success "URLs corrigidas de forma direta!"
}
...
verify_fixes() {
    ...
    local https_count=$(find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" \) -exec grep -l "https://$DOMAIN" {} \; | wc -l)
    local localhost_count=$(find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" \) -exec grep -l "localhost:3001\|localhost:8080\|localhost:3000" {} \; | wc -l)
    ...
    if [ "$https_count" -eq 0 ] && [ "$localhost_count" -eq 0 ]; then
        ...
        if [ "$https_count" -gt 0 ]; then
            ...
            find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" \) -exec grep -l "https://$DOMAIN" {} \;
        fi
        
        if [ "$localhost_count" -gt 0 ]; then
            log_info "Arquivos com localhost:"
            find dist -type f \( -name "*.js" -o -name "*.html" -o -name "*.css" \) -exec grep -l "localhost:3001\|localhost:8080\|localhost:3000" {} \;
        fi
    fi
}
...
show_final_info() {
    log_header "Correção Direta Concluída!"
    
    echo -e "\n${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║                                                              ║${NC}"
    echo -e "${GREEN}║           ✅ CORREÇÃO DIRETA DE URLs CONCLUÍDA!             ║${NC}"
    echo -e "${GREEN}║                                                              ║${NC}"
    echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
    ...
    echo -e "✅ URLs HTTPS corrigidas para HTTP"
    echo -e "✅ URLs localhost corrigidas para domínio"
...
# Função principal
main() {
    log_header "Iniciando correção direta de URLs hardcoded..."
