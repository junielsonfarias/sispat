...
async function fazerLogin() {
  ...
  if (result.success && result.data?.token) {
    ...
    console.log(
      '🏛️ Município:',
      result.data.user.municipality_name || 'Não definido'
    );
...
async function verificarMunicipio(token) {
  ...
  if (userResult.success && userResult.data?.municipality_id) {
    console.log(
      '✅ Usuário já tem município:',
      userResult.data.municipality_name
    );
  ...
  if (result.success && result.data?.id) {
    ...
    if (updateResult.success) {
      ...
      console.log(
        '⚠️ Não foi possível atualizar usuário:',
        updateResult.data?.error || updateResult.error
      );
    }
...
async function verificarSetores(token, municipalityId) {
  ...
  const result = await makeRequest(
    `${CONFIG.BASE_URL}/api/sectors?municipalityId=${municipalityId}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  if (createResult.success && createResult.data?.id) {
    ...
    console.log(
      ...
      createResult.data?.error || createResult.error
    );
    return null;
  }
}
...
async function verificarLocais(token, municipalityId, sectorId) {
  ...
  const result = await makeRequest(
    `${CONFIG.BASE_URL}/api/locals?municipalityId=${municipalityId}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
async function testarEdicaoBem(token, bemId) {
  ...
  const dadosEditados = {
    ...
    valor_aquisicao: 2600.0,
    status: 'manutencao',
  };

  const result = await makeRequest(
    `${CONFIG.BASE_URL}/api/patrimonios/${bemId}`,
    {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
...
async function main() {
  ...
  try {
    ...
    if (!stats.frontend) {
      console.log(
        '⚠️ Frontend não está funcionando, mas continuando com testes da API'
      );
    }
    ...
    if (!municipalityId) {
      console.log(
        '❌ Teste interrompido: não foi possível obter/criar município'
      );
      return;
    }
    ...
    const bemId = await testarCriacaoBem(
      token,
      municipalityId,
      sectorId,
      localId
