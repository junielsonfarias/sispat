...
const backend = startProcess('npm', ['run', 'dev:backend'], 'Backend', colors.blue);

// Aguardar um pouco para o backend inicializar
setTimeout(() => {
  log.header('🎯 Iniciando Frontend...');
  const frontend = startProcess('npm', ['run', 'dev:frontend'], 'Frontend', colors.green);
  
  // Mostrar informações de acesso
  setTimeout(() => {
    log.header('🎉 SISPAT Iniciado com Sucesso!');
    console.log(`\n${colors.cyan}📋 INFORMAÇÕES DE ACESSO:${colors.reset}`);
    console.log(`🌐 Frontend: ${colors.yellow}http://localhost:8080${colors.reset}`);
    console.log(`🔧 Backend:  ${colors.yellow}http://localhost:3001${colors.reset}`);
    console.log(`📊 API:      ${colors.yellow}http://localhost:3001/api${colors.reset}`);
    ...
    console.log(`📊 Status: ${colors.yellow}netstat -an | findstr :3001${colors.reset}`);
    console.log(`🔍 Logs:   ${colors.yellow}Get-Content logs/error.log -Tail 10${colors.reset}`);
    console.log(`🔄 Restart: ${colors.yellow}Ctrl+C e execute novamente${colors.reset}`);
    
    console.log(`\n${colors.green}✅ Sistema funcionando em modo desenvolvimento (sem banco de dados)${colors.reset}`);
    console.log(`${colors.blue}💡 Para usar com banco de dados, configure PostgreSQL e altere DISABLE_DATABASE=false${colors.reset}\n`);
  }, 3000);
}, 2000);
