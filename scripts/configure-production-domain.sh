...
# =================================
# CONFIGURAÇÃO AUTOMÁTICA DE DOMÍNIO - SISPAT
# Script para configurar automaticamente o domínio em produção
# =================================
...
echo ""
echo "🌐 ================================================"
echo "🌐    CONFIGURAÇÃO AUTOMÁTICA DE DOMÍNIO - SISPAT"
echo "🌐    Configuração automática para ambiente VPS"
echo "🌐 ================================================"
echo ""
...
# 1. Detectar domínio automaticamente
log "🔍 Detectando domínio automaticamente..."

# Tentar detectar domínio de várias formas
DOMAIN=""

# Método 1: Verificar variável de ambiente HOSTNAME
if [ ! -z "$HOSTNAME" ] && [[ "$HOSTNAME" != "localhost" ]] && [[ "$HOSTNAME" != "127.0.0.1" ]]; then
    DOMAIN="$HOSTNAME"
    log "📡 Domínio detectado via HOSTNAME: $DOMAIN"
fi

# Método 2: Verificar arquivo /etc/hostname
if [ -z "$DOMAIN" ] && [ -f "/etc/hostname" ]; then
    HOSTNAME_FROM_FILE=$(cat /etc/hostname | tr -d '\n' | tr -d '\r')
    if [[ "$HOSTNAME_FROM_FILE" != "localhost" ]] && [[ "$HOSTNAME_FROM_FILE" != "127.0.0.1" ]]; then
        DOMAIN="$HOSTNAME_FROM_FILE"
        log "📡 Domínio detectado via /etc/hostname: $DOMAIN"
    fi
fi

# Método 3: Verificar configuração do Nginx
if [ -z "$DOMAIN" ] && [ -f "/etc/nginx/sites-available/sispat" ]; then
    ...
    if [ ! -z "$NGINX_DOMAIN" ] && [[ "$NGINX_DOMAIN" != "localhost" ]]; then
        DOMAIN="$NGINX_DOMAIN"
        log "📡 Domínio detectado via Nginx: $DOMAIN"
    fi
fi
...
if [ -z "$DOMAIN" ] && [ -d "/etc/letsencrypt/live" ]; then
    ...
    if [ ! -z "$SSL_DOMAIN" ]; then
        DOMAIN="$SSL_DOMAIN"
        log "📡 Domínio detectado via SSL: $DOMAIN"
    fi
fi

# Método 5: Solicitar domínio do usuário
if [ -z "$DOMAIN" ]; then
    echo ""
    warning "⚠️ Não foi possível detectar o domínio automaticamente"
    echo ""
    read -p "🌐 Digite o domínio da sua aplicação (ex: sispat.exemplo.com): " DOMAIN
...
success "✅ Domínio configurado: $DOMAIN"

# 2. Determinar protocolo (HTTP ou HTTPS)
...
# Criar nova configuração do Vite com domínio dinâmico
...
# 6. Atualizar configuração do servidor
log "📝 Atualizando configuração do servidor..."

# Adicionar configuração de CORS dinâmica no servidor
if grep -q "CORS_ORIGIN" server/index.js; then
    # Atualizar CORS existente
    sed -i "s|CORS_ORIGIN=.*|CORS_ORIGIN=$BASE_URL,http://localhost:3000,http://127.0.0.1:3000,http://localhost:8080,http://127.0.0.1:8080|g" server/index.js
else
    # Adicionar configuração CORS se não existir
    echo "" >> server/index.js
    echo "// Configuração CORS dinâmica para produção" >> server/index.js
    echo "const corsOptions = {" >> server/index.js
    echo "  origin: process.env.CORS_ORIGIN || '$BASE_URL,http://localhost:3000,http://127.0.0.1:3000,http://localhost:8080,http://127.0.0.1:8080'," >> server/index.js
    echo "  credentials: true," >> server/index.js
    echo "  optionsSuccessStatus: 200" >> server/index.js
    echo "};" >> server/index.js
fi

success "✅ Configuração do servidor atualizada"

# 7. Fazer rebuild com novas configurações
...
# 8. Verificar se o build foi criado corretamente
...
if curl -s -o /dev/null -w "%{http_code}" "$BASE_URL" | grep -q "200"; then
    success "✅ Frontend acessível em $BASE_URL"
else
    warning "⚠️ Frontend pode não estar acessível em $BASE_URL"
fi

# Testar API
if curl -s -o /dev/null -w "%{http_code}" "$API_URL/health" | grep -q "200"; then
    success "✅ API acessível em $API_URL"
else
    warning "⚠️ API pode não estar acessível em $API_URL"
fi

# Instruções finais
log "📝 CONFIGURAÇÃO AUTOMÁTICA DE DOMÍNIO CONCLUÍDA!"
echo ""
echo "🎉 DOMÍNIO CONFIGURADO AUTOMATICAMENTE!"
echo "========================================="
echo ""
...
echo "✅ server/index.js - CORS configurado"
...
echo "   1. Teste a aplicação no navegador: $BASE_URL"
...
success "🎉 Configuração automática de domínio concluída!"
