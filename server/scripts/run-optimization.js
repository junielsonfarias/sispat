...
const main = async () => {
  try {
    ...
    console.log(`\n✅ Análise concluída:`)
    console.log(`   • Queries lentas encontradas: ${analysis.slowQueries?.length || 0}`)
    console.log(`   • Índices existentes: ${analysis.existingIndexes?.length || 0}`)
    console.log(`   • Índices não utilizados: ${analysis.indexUsage?.unusedIndexes?.length || 0}`)
    console.log(`   • Tabelas precisando manutenção: ${analysis.tableStats?.needMaintenance?.length || 0}`)
    console.log(`   • Novos índices sugeridos: ${analysis.indexSuggestions?.length || 0}`)
    ...
    console.log(`   • Índices criados: ${optimization.indexCreation?.created || 0}`)
    console.log(`   • Índices já existentes: ${optimization.indexCreation?.skipped || 0}`)
    console.log(`   • Índices com falha: ${optimization.indexCreation?.failed || 0}`)
    console.log(`   • Tabelas com manutenção: ${optimization.tableMaintenance?.maintained || 0}`)
    console.log(`   • Tempo total: ${(optimization.totalDuration / 1000).toFixed(2)}s`)
    ...
    if (analysis.slowQueries?.length > 0) {
      ...
      analysis.slowQueries.slice(0, 5).forEach((query, i) => {
        ...
        console.log(`      Tempo médio: ${parseFloat(query.mean_time).toFixed(2)}ms`)
    ...
    console.log('\n' + '='.repeat(60))
    console.log('🎉 Otimização do banco de dados SISPAT concluída com sucesso!')
