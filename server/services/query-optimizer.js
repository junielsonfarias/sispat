...
class QueryOptimizer {
    ...
    try {
      ...
      if (analyze) {
        ...
        logInfo('Query execution plan', {
          ...
          totalTime:
            executionPlan['Execution Time'] + executionPlan['Planning Time'],
        });
      }
  ...
  trackQueryStats(sql, duration, rowCount) {
    ...
    if (duration > this.slowQueryThreshold) {
      this.slowQueries.set(queryKey, {
        ...
        slowExecutions:
          (this.slowQueries.get(queryKey)?.slowExecutions || 0) + 1,
      });
    }
  }
    ...
    return {
      summary: {
        ...
        slowQueriesPercentage:
          totalQueries > 0
            ? ((slowQueriesCount / totalQueries) * 100).toFixed(2)
            : 0,
      },
  ...
  async suggestOptimizations(sql, params = []) {
    try {
      const result = await this.executeWithAnalysis(sql, params, {
        analyze: true,
      });
      ...
      if (result.rowCount > 1000) {
        suggestions.push({
          ...
          recommendation:
            'Consider implementing pagination to limit result set',
        });
      }
  ...
  async runSystemAnalysis() {
    ...
    for (const sql of commonQueries) {
      try {
        const analysis = await this.suggestOptimizations(sql, [
