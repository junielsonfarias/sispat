...
class CacheManager {
  ...
  async initialize() {
    try {
      ...
      if (redisClient && typeof redisClient.isOpen === 'function' && redisClient.isOpen()) {
  ...
  async get(key) {
    ...
    try {
      ...
      if (this.isRedisAvailable && redisClient && typeof redisClient.get === 'function') {
        try {
      ...
      if (value) {
        this.stats.hits++
        logPerformance(`Cache HIT: ${key}`, duration, { cacheType: this.isRedisAvailable ? 'redis' : 'memory' })
        return value
      } else {
        this.stats.misses++
        logPerformance(`Cache MISS: ${key}`, duration, { cacheType: this.isRedisAvailable ? 'redis' : 'memory' })
        return null
      }
    ...
    try {
      if (this.isRedisAvailable) {
        ...
        if (keysToDelete.length > 0) {
          logInfo(`Cache DELETE PATTERN: ${pattern} (${keysToDelete.length} chaves)`)
        }
      }
  ...
  cleanOldestMemoryEntries() {
    ...
    logInfo(`Cache memory cleanup: ${entries.length} entradas antigas removidas`)
  }
...
export const cache = {
  ...
  async getPatrimonios(municipalityId, filters = {}) {
    ...
    const key = cacheManager.generateKey('patrimonios', filterKey, null, municipalityId)
    return await cacheManager.get(key)
  },
  ...
  async getSectors(municipalityId) {
    const key = cacheManager.generateKey('sectors', 'all', null, municipalityId)
    return await cacheManager.get(key)
  },
