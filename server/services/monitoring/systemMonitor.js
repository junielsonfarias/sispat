...
class SystemMonitor {
  ...
  checkThresholds() {
    ...
    if (avgResponseTime > this.thresholds.responseTime) {
      this.createAlert(
        'SLOW_RESPONSE',
        `Tempo de resposta lento: ${avgResponseTime.toFixed(2)}ms`
      );
    }
  ...
  async checkDatabaseHealth() {
    try {
      ...
      this.createAlert(
        'DATABASE_CONNECTION',
        `Erro de conexão com banco: ${error.message}`
      );
    ...
    setInterval(
      () => {
        ...
        this.alerts = this.alerts.filter(
          alert => new Date(alert.timestamp) > oneHourAgo || !alert.acknowledged
        );
      },
      60 * 60 * 1000
    );
