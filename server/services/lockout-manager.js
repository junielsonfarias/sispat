...
export class LockoutManager {
  ...
  async recordLoginAttempt(data) {
    try {
      ...
      `, [userId, email, ipAddress, userAgent, success, failureReason, municipalityId])
   ...
   */
  async handleFailedLogin(user, ipAddress, userAgent, failureReason = 'invalid_credentials') {
    try {
      ...
      if (newFailedAttempts >= this.config.maxAttempts) {
        ...
        logSecurity('User account locked due to multiple failed attempts', 'error', {
          userId: user.id,
          email: user.email,
          ipAddress,
      ...
      return {
        isLocked,
        remainingAttempts: Math.max(0, this.config.maxAttempts - newFailedAttempts),
        lockoutUntil,
        failedAttempts: newFailedAttempts
      }

    } catch (error) {
      logError('Erro ao processar login falhado', error, { userId: user.id, email: user.email })
      throw error
    }
  }
  ...
  async handleSuccessfulLogin(user, ipAddress, userAgent) {
    try {
      ...
      logError('Erro ao processar login bem-sucedido', error, { userId: user.id })
    }
  }
    ...
    try {
      ...
      if (result.rowCount > 0) {
        logInfo('Usuários desbloqueados automaticamente', {
          ...
          users: result.rows.map(u => ({ id: u.id, email: u.email, name: u.name }))
        })
