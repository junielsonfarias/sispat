  ...
  async connect() {
    ...
    try {
      ...
      if (this.retryCount < this.maxRetries) {
        logInfo(
          `Tentando reconectar em 5 segundos... (${this.retryCount}/${this.maxRetries})`
        );
        setTimeout(() => this.connect(), 5000);
      } else {
        logError(
          'Máximo de tentativas de conexão Redis atingido - funcionando sem cache'
        );
      }
...
if (
  ...
  redisClient.connect().catch(error => {
    logWarning('Redis não disponível - funcionando sem cache', {
      error: error.message,
    });
  });
} else {
