...
export class BackupScheduler {
  ...
  start() {
    ...
    try {
      ...
      const verificationJob = cron.schedule(
        '0 1 * * *',
        async () => {
  ...
  async checkInitialBackup() {
    try {
      ...
      if (!recentBackup) {
        logInfo(
          '🔄 Nenhum backup recente encontrado - executando backup inicial'
        );
        await this.executeDailyBackup();
      } else {
        logInfo('✅ Backup recente encontrado', {
          ...
          age:
            Math.round(
              (now - new Date(recentBackup.created)) / (1000 * 60 * 60)
            ) + 'h',
        });
      }
    } catch (error) {
  ...
  async executeDailyBackup() {
    ...
    try {
      ...
      await this.sendNotification('failure', 'daily', {
        error: error.message,
        duration,
      });
  ...
  async executeWeeklyBackup() {
    try {
      ...
      await this.sendNotification('failure', 'weekly', {
        error: error.message,
      });
  ...
  async executeMonthlyBackup() {
    try {
      ...
      await this.sendNotification('failure', 'monthly', {
        error: error.message,
      });
    ...
    try {
      ...
      for (const backup of recentBackups) {
        try {
          ...
          failedCount++;
          logError('❌ Erro ao verificar backup', error, {
            file: backup.filename,
          });
        }
      }
  ...
  async tryFallbackBackup(type) {
    try {
      ...
      await this.sendNotification('fallback_failure', type, {
        error: error.message,
      });
    }
  }
