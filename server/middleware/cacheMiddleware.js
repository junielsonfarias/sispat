    ...
    try {
      ...
      res.json = function(data) {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          cacheManager.set(cacheKey, data, ttlType)
            .catch(error => logError('Erro ao armazenar cache via middleware', error))
        }
 ...
 */
export const patrimoniosCacheMiddleware = cacheMiddleware('patrimonios', (req) => {
  const municipalityId = req.user?.municipality_id
  const query = req.query
  const filters = Object.keys(query).length > 0 ? JSON.stringify(query) : 'all'
  
  return cacheManager.generateKey('patrimonios', filters, null, municipalityId)
})
...
export const reportsCacheMiddleware = cacheMiddleware('reports', (req) => {
  ...
  return cacheManager.generateKey('reports', `${reportType}:${filters}`, null, municipalityId)
})
...
export const cacheInvalidationMiddleware = (invalidationType) => {
  return (req, res, next) => {
    ...
    res.json = function(data) {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        invalidateCache(invalidationType, req)
          .catch(error => logError('Erro ao invalidar cache via middleware', error))
      }
 ...
 */
export const invalidatePatrimoniosCache = cacheInvalidationMiddleware('patrimonios')
