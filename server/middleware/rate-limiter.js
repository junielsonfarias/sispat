...
class AdvancedRateLimiter {
    ...
    try {
      ...
      if (process.env.REDIS_URL && process.env.REDIS_URL !== 'redis://localhost:6379') {
        try {
          ...
          logWarning('Redis não disponível, usando fallback de memória', { error: error.message })
        }
      }
      ...
      logWarning('Inicializando rate limiter apenas com memória como fallback final')
      ...
      this.initialized = true
      logInfo('Rate limiting inicializado em modo de emergência (apenas memória)')
    }
  }
  ...
  isWhitelisted(ip) {
    return WHITELIST_IPS.has(ip) || 
           WHITELIST_IPS.has(ip.replace(/^::ffff:/, '')) // IPv4 mapeado em IPv6
  }
  ...
  getLimiterType(req) {
    ...
    if (path.includes('/api/uploads') || 
        (method === 'POST' && req.headers['content-type']?.includes('multipart/form-data'))) {
      return 'upload'
    }
    ...
    if (path.includes('/api/patrimonios') || 
  ...
  middleware() {
    return async (req, res, next) => {
      try {
        ...
        if (!limiter) {
          logWarning('Rate limiter não encontrado para tipo', { type: limiterType })
          return next()
        }
      ...
      for (const [type, limiter] of this.limiters) {
        try {
          ...
          stats[type] = {
            ...
            remainingPoints: result?.remainingPoints || RATE_LIMIT_CONFIGS[type].points,
            msBeforeNext: result?.msBeforeNext || 0,
            isBlocked: (result?.remainingPoints || RATE_LIMIT_CONFIGS[type].points) <= 0
          }
        } catch (error) {
