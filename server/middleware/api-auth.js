...
export const authenticateApiKey = async (req, res, next) => {
  try {
    ...
    if (result.rows.length === 0) {
      logWarning('Tentativa de acesso com API key inválida', { apiKey: apiKey.substring(0, 8) + '...' })
      return res.status(401).json({
        success: false,
...
export const createApiKey = async (name, userId, permissions = [], rateLimitPerHour = 1000) => {
  try {
