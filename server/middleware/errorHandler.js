...
export class AppError extends Error {
  constructor(message, statusCode = 500, errorCode = ERROR_CODES.INTERNAL_ERROR, details = {}) {
    super(message)
    this.name = 'AppError'
...
export const notFoundHandler = (req, res, next) => {
  const error = new AppError(
    ...
    ERROR_CODES.NOT_FOUND,
    {
      method: req.method,
      path: req.path,
      availableRoutes: req.app._router ? req.app._router.stack.map(r => r.route?.path).filter(Boolean) : [],
    }
  )
