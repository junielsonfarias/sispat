...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (req.user.role !== 'superuser' && inventory.municipality_id !== req.user.municipality_id) {
...
router.post('/', requireUser, async (req, res) => {
  try {
    ...
    await query(
      ...
      [req.user.id, req.user.name, 'INVENTORY_CREATE', `Inventário "${name}" criado.`, municipalityId]
    )
...
router.put('/:id', requireUser, async (req, res) => {
  try {
    ...
    // Check if user has access to this inventory's municipality
    if (req.user.role !== 'superuser' && existingInventory.municipality_id !== req.user.municipality_id) {
      return res.status(403).json({ error: 'Acesso negado' })
    }
    ...
    const result = await query(`
      ...
      RETURNING *
    `, [
      updateData.name, updateData.sectorName, updateData.scope, 
      updateData.locationType, updateData.status, updateData.finalizedAt, id
    ])
    ...
    await query(
      'INSERT INTO activity_logs (user_id, user_name, action, details, municipality_id) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, req.user.name, 'INVENTORY_UPDATE', `Inventário "${updatedInventory.name}" atualizado.`, existingInventory.municipality_id]
    )
...
router.delete('/:id', requireUser, async (req, res) => {
  try {
    ...
    await query(
      'INSERT INTO activity_logs (user_id, user_name, action, details, municipality_id) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, req.user.name, 'INVENTORY_DELETE', `Inventário "${existingInventory.name}" excluído.`, existingInventory.municipality_id]
    )
