  ...
  try {
    ...
    if (user.locked_until && new Date() < new Date(user.locked_until)) {
      return res.status(423).json({ 
        ...
        message: 'Sua conta foi bloqueada por segurança. Tente novamente mais tarde ou contate um administrador.'
      })
    }
    ...
    try {
      await query(
        ...
        [user.id, 'LOGIN_SUCCESS', 'users', user.id, JSON.stringify({ email: user.email, role: user.role }), user.municipality_id]
      )
...
router.post('/logout', authenticateToken, async (req, res) => {
  try {
    ...
    await query(
      'INSERT INTO activity_logs (user_id, action, table_name, record_id, new_values, municipality_id) VALUES ($1, $2, $3, $4, $5, $6)',
      [req.user.id, 'LOGOUT', 'users', req.user.id, JSON.stringify({ action: 'logout' }), req.user.municipality_id]
    )
...
// API Keys routes
import { createApiKey, createApiKeysTable, listApiKeys, revokeApiKey } from '../middleware/api-auth.js'
