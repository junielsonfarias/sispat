...
router.post('/', requireSuperuser, async (req, res) => {
  try {
    ...
    console.log('Create municipality - Request body:', JSON.stringify(req.body, null, 2))
    console.log('Create municipality - Content-Type:', req.headers['content-type'])
    ...
    // Validation
    if (!name || !fullAddress || !cnpj || !contactEmail || !mayorName || !mayorCpf) {
    ...
    console.log('Create municipality - Inserting with values:', {
      name, state, logoUrl, fullAddress, cnpj, contactEmail,
      mayorName, mayorCpf, accessStartDate, accessEndDate
    })
    ...
    await query(
      ...
      [req.user.id, 'MUNICIPALITY_CREATE', 'municipalities', municipality.id, JSON.stringify({ name }), municipality.id]
    )
...
router.put('/:id', requireSuperuser, async (req, res) => {
  try {
    ...
    const result = await query(`
      ...
      RETURNING *
    `, [
      name, logoUrl, fullAddress, cnpj, contactEmail,
      mayorName, mayorCpf, accessStartDate, accessEndDate, id
    ])
    ...
    await query(
      'INSERT INTO activity_logs (user_id, action, table_name, record_id, new_values, municipality_id) VALUES ($1, $2, $3, $4, $5, $6)',
      [req.user.id, 'MUNICIPALITY_UPDATE', 'municipalities', municipality.id, JSON.stringify({ name: municipality.name }), municipality.id]
    )
...
router.post('/:id/assign-supervisor', async (req, res) => {
  try {
    ...
    await query(
      'INSERT INTO activity_logs (user_id, action, table_name, record_id, new_values, municipality_id) VALUES ($1, $2, $3, $4, $5, $6)',
      [req.user.id, 'SUPERVISOR_ASSIGNED', 'municipalities', id, JSON.stringify({ supervisor_name: supervisor.name }), id]
    )
...
router.delete('/:id/remove-supervisor', async (req, res) => {
  try {
    ...
    if (!municipality.supervisor_id) {
      return res.status(400).json({ error: 'Município não possui supervisor atribuído' })
    }
    ...
    await query(
      'UPDATE users SET municipality_id = NULL WHERE id = $1',
      [municipality.supervisor_id]
    )
    ...
    await query(
      'INSERT INTO activity_logs (user_id, action, table_name, record_id, new_values, municipality_id) VALUES ($1, $2, $3, $4, $5, $6)',
      [req.user.id, 'SUPERVISOR_REMOVED', 'municipalities', id, JSON.stringify({ supervisor_name: supervisor.name }), id]
    )
...
// Delete municipality - FORCE MODE FOR DEVELOPMENT
  router.delete('/:id', requireSuperuser, async (req, res) => {
    try {
      ...
      console.log(`🗑️ Found municipality: ${municipality.name} - Starting FORCE deletion...`)
      ...
      try {
        ...
        const deleteQueries = [
          ...
          { table: 'patrimonio_notes', query: 'DELETE FROM patrimonio_notes WHERE patrimonio_id IN (SELECT id FROM patrimonios WHERE municipality_id = $1)' },
          { table: 'inventory_items', query: 'DELETE FROM inventory_items WHERE inventory_id IN (SELECT id FROM inventories WHERE municipality_id = $1)' },
          { table: 'transfers', query: 'DELETE FROM transfers WHERE municipality_id = $1' },
          
          // Second: Remove activity and audit logs
          { table: 'activity_logs', query: 'DELETE FROM activity_logs WHERE municipality_id = $1' },
          { table: 'audit_logs', query: 'DELETE FROM audit_logs WHERE municipality_id = $1' },
          { table: 'notifications', query: 'DELETE FROM notifications WHERE municipality_id = $1' },
          { table: 'lockout_records', query: 'DELETE FROM lockout_records WHERE municipality_id = $1' },
          
          // Third: Remove analytics and reports
          { table: 'analytics_metrics', query: 'DELETE FROM analytics_metrics WHERE municipality_id = $1' },
          { table: 'report_queue', query: 'DELETE FROM report_queue WHERE municipality_id = $1' },
          
          // Fourth: Remove main data tables
          { table: 'patrimonios', query: 'DELETE FROM patrimonios WHERE municipality_id = $1' },
          { table: 'imoveis', query: 'DELETE FROM imoveis WHERE municipality_id = $1' },
          { table: 'inventories', query: 'DELETE FROM inventories WHERE municipality_id = $1' },
          
          // Fifth: Remove configuration tables
          { table: 'themes', query: 'DELETE FROM themes WHERE municipality_id = $1' },
          { table: 'report_templates', query: 'DELETE FROM report_templates WHERE municipality_id = $1' },
          { table: 'label_templates', query: 'DELETE FROM label_templates WHERE municipality_id = $1' },
          { table: 'locals', query: 'DELETE FROM locals WHERE municipality_id = $1' },
          { table: 'sectors', query: 'DELETE FROM sectors WHERE municipality_id = $1' },
          
          // Last: Remove users (except superuser)
          { table: 'users', query: 'DELETE FROM users WHERE municipality_id = $1 AND role != $2', params: [id, 'superuser'] }
        ]

        // Execute all delete queries
        for (const { table, query: deleteQuery, params = [id] } of deleteQueries) {
          try {
        ...
        const municipalityResult = await query('DELETE FROM municipalities WHERE id = $1', [id])
        console.log(`✅ Municipality deleted: ${municipalityResult.rowCount} rows`)
        ...
        res.json({ 
          success: true,
          message: 'Município excluído com sucesso (FORÇADO - MODO DESENVOLVIMENTO)',
