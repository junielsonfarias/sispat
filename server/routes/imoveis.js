...
router.get('/public', sanitizeSearchInput, async (req, res) => {
  try {
    ...
    if (!hasMunicipalityId) {
      console.log(
        'ℹ️ Tabela imoveis não tem coluna municipality_id, retornando lista vazia'
      );
      return res.json([]);
    }
    ...
    if (search && municipalityId) {
      whereClause +=
        ' AND (i.numero_patrimonio ILIKE $2 OR i.endereco ILIKE $2 OR i.descricao ILIKE $2)';
      params.push(`%${search}%`);
    } else if (search) {
      whereClause =
        'WHERE (i.numero_patrimonio ILIKE $1 OR i.endereco ILIKE $1 OR i.descricao ILIKE $1)';
      params.push(`%${search}%`);
    }
    ...
    if (existingColumns.has('area_construida'))
      selectColumns.push('i.area_construida');
    if (existingColumns.has('valor_aquisicao'))
      selectColumns.push('i.valor_aquisicao');
    if (existingColumns.has('data_aquisicao'))
      selectColumns.push('i.data_aquisicao');
...
router.get('/public/:id', async (req, res) => {
  try {
    ...
    if (existingColumns.has('numero_patrimonio'))
      selectColumns.push('i.numero_patrimonio');
    if (existingColumns.has('endereco')) selectColumns.push('i.endereco');
    if (existingColumns.has('descricao')) selectColumns.push('i.descricao');
    if (existingColumns.has('municipality_id'))
      selectColumns.push('i.municipality_id');
    if (existingColumns.has('area_total')) selectColumns.push('i.area_total');
    if (existingColumns.has('area_construida'))
      selectColumns.push('i.area_construida');
    if (existingColumns.has('valor_aquisicao'))
    ...
    console.log(
      `✅ Imóvel encontrado: ${imovel.endereco || imovel.numero_patrimonio}`
    );
    res.json(imovel);
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (
      ...
      imovel.municipality_id !== req.user.municipality_id
    ) {
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          'Campos obrigatórios: número do patrimônio, denominação, endereço, setor, data de aquisição e valor',
      });
    }
    ...
    await query(
      ...
      [
        ...
        req.user.name,
        'IMOVEL_CREATE',
        `Imóvel "${numero_patrimonio}" criado.`,
        municipalityId,
      ]
    );
...
router.put('/:id', requireUser, async (req, res) => {
  try {
    ...
    if (
      req.user.role !== 'superuser' &&
      existingImovel.municipality_id !== req.user.municipality_id
    ) {
      return res.status(403).json({ error: 'Acesso negado' });
    }
    ...
    await query(
      ...
      [
        ...
        req.user.name,
        'IMOVEL_UPDATE',
        `Imóvel "${updatedImovel.numero_patrimonio}" atualizado.`,
        existingImovel.municipality_id,
      ]
    );
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        req.user.name,
        'IMOVEL_DELETE',
        `Imóvel "${existingImovel.numero_patrimonio}" excluído.`,
        existingImovel.municipality_id,
      ]
    );
