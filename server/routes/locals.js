...
router.get('/user-sectors', async (req, res) => {
  try {
    console.log('🔍 Buscando locais dos setores do usuário:', req.user?.id, req.user?.role)
    ...
    if (req.user.role === 'superuser' || req.user.role === 'supervisor') {
      console.log('🔍 Usuário é supervisor/superusuário, buscando todos os locais do município')
    ...
    console.log('🔍 Nomes dos setores do usuário:', userSectors.map(s => s.sector_name))
    ...
    for (const sectorId of sectorIds) {
      try {
        ...
        console.error(`❌ Erro ao buscar locais do setor ${sectorId}:`, sectorError)
        // Continua com os outros setores
      }
    }
    
    console.log('✅ Locais encontrados para os setores do usuário:', locals.length)
    res.json(locals)
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (req.user.role !== 'superuser' && local.municipalityId !== req.user.municipality_id) {
    ...
    // Check if user has access to this local's municipality
    if (req.user.role !== 'superuser' && local.municipality_id !== req.user.municipality_id) {
      return res.status(403).json({ error: 'Acesso negado' })
    }
  ...
  try {
    ...
    try {
      ...
      await query(
        ...
        [req.user.id, req.user.name, 'LOCAL_CREATE', `Local "${name}" criado.`, municipalityId]
      )
      console.log('✅ Atividade registrada com sucesso')
    } catch (activityError) {
      console.log('⚠️ Erro ao registrar atividade (ignorando):', activityError.message)
...
router.put('/:id', requireSupervisor, async (req, res) => {
  try {
    ...
    // Check if user has access to this local's municipality
    if (req.user.role !== 'superuser' && existingLocal.municipality_id !== req.user.municipality_id) {
      return res.status(403).json({ error: 'Acesso negado' })
    }
    ...
    await query(
      'INSERT INTO activity_logs (user_id, user_name, action, details, municipality_id) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, req.user.name, 'LOCAL_UPDATE', `Local "${updatedLocal.name}" atualizado.`, existingLocal.municipality_id]
    )
...
router.delete('/:id', requireSupervisor, async (req, res) => {
  try {
    ...
    if (parseInt(patrimonios.count) > 0) {
      return res.status(400).json({ 
        error: 'Não é possível excluir um local que possui patrimônios vinculados' 
      })
    }
    ...
    await query(
      'INSERT INTO activity_logs (user_id, user_name, action, details, municipality_id) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, req.user.name, 'LOCAL_DELETE', `Local "${existingLocal.name}" excluído.`, existingLocal.municipality_id]
    )
