    ...
    `;
      ...
      `,
        ...
        if (userSectors.length > 0) {
          ...
          console.log(
            '⚠️ Usuário sem setores atribuídos - sem acesso a setores'
          );
          return res.json([]);
        }
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!municipalityId) {
        return res.status(400).json({
          error:
            'Município é obrigatório. Verifique se você está vinculado a um município.',
        });
      }
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (
        req.user.role !== 'superuser' &&
        existingSector.municipality_id !== req.user.municipality_id
      ) {
        return res.status(403).json({ error: 'Acesso negado' });
      }
    ...
    `,
      ...
      await query(
        ...
        [
          ...
          existingSector.municipality_id,
        ]
      );
    ...
    try {
      ...
      try {
        ...
        if (patrimonios && parseInt(patrimonios.count) > 0) {
          return res.status(400).json({
            error:
              'Não é possível excluir um setor que possui patrimônios vinculados',
          });
        }
      } catch (error) {
      ...
      try {
        ...
        if (locals && parseInt(locals.count) > 0) {
          return res.status(400).json({
            error:
              'Não é possível excluir um setor que possui locais vinculados',
          });
        }
      } catch (error) {
      ...
      await query(
        ...
        [
          ...
          'sectors',
          id,
          JSON.stringify({ name: existingSector.name }),
          existingSector.municipality_id,
        ]
      );
...
router.get('/debug/check-sectors', async (req, res) => {
  try {
    ...
    const sectorsWithoutCode = sectors.filter(
      s => !s.codigo || s.codigo === ''
    );
