...
router.get('/system-health', requireAdmin, async (req, res) => {
  try {
    const systemHealth = analyticsEngine.getMetrics(
      'system_health',
      null,
      '1h'
    );
    const performanceMetrics = analyticsEngine.getMetrics(
      'performance_metrics',
      null,
      '1h'
    );
    ...
    const latest =
      systemHealth.length > 0 ? systemHealth[systemHealth.length - 1] : null;
    const latestPerf =
...
router.get('/export', requireAdmin, async (req, res) => {
  try {
    const { type = 'all', format = 'json', timeRange = '24h' } = req.query;
...
async function calculateKPIs(municipalityId) {
  try {
    // KPIs de patrimônios
    const patrimonyMetrics = analyticsEngine.getMetrics(
      'patrimony_stats',
      municipalityId,
      '24h'
    );
    const latestPatrimony =
      patrimonyMetrics.length > 0
        ? patrimonyMetrics[patrimonyMetrics.length - 1]
        : null;

    // KPIs de usuários
    const userMetrics = analyticsEngine.getMetrics(
      'user_activity',
      municipalityId,
      '24h'
    );
    ...
    const transferMetrics = analyticsEngine.getMetrics(
      'transfer_activity',
      municipalityId,
      '24h'
    );
    const latestTransfers =
      transferMetrics.length > 0
    ...
    return {
      patrimony: {
        ...
        trend: analyticsEngine.calculateTrend(
          patrimonyMetrics.map(m => m.value)
        ),
      },
      users: {
