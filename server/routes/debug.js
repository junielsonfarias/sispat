...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        '🧪 Testando exclusão real de município:',
        id,
        'force:',
        force
      );
      ...
      if (dependencies.hasDependencies && force !== 'true') {
        return res.status(400).json({
          success: false,
          error:
            'Não é possível excluir o município. Existem dados vinculados a ele.',
      ...
      if (dependencies.hasDependencies && force === 'true') {
        ...
        try {
          ...
          if (dependencies.details.some(d => d.includes('Logs de atividade'))) {
            ...
            await query(
              'DELETE FROM activity_logs WHERE municipality_id = $1',
              [id]
            );
          }
          ...
          if (
            dependencies.details.some(d =>
              d.includes('Templates de relatórios')
            )
          ) {
            console.log('🧪 Removendo templates de relatórios...');
            await query(
              'DELETE FROM report_templates WHERE municipality_id = $1',
              [id]
            );
          }

          if (
            dependencies.details.some(d => d.includes('Templates de etiquetas'))
          ) {
            console.log('🧪 Removendo templates de etiquetas...');
            await query(
              'DELETE FROM label_templates WHERE municipality_id = $1',
              [id]
            );
          }
          ...
          if (dependencies.details.some(d => d.includes('Inventários'))) {
            ...
            await query('DELETE FROM inventories WHERE municipality_id = $1', [
              id,
            ]);
          }
          ...
          if (dependencies.details.some(d => d.includes('Patrimônios'))) {
            ...
            await query('DELETE FROM patrimonios WHERE municipality_id = $1', [
              id,
            ]);
          }
          ...
          if (dependencies.details.some(d => d.includes('Usuários'))) {
            ...
            await query(
              'DELETE FROM users WHERE municipality_id = $1 AND role != $2',
              [id, 'superuser']
            );
          }
        } catch (cleanupError) {
      ...
      try {
        ...
        const result = await query('DELETE FROM municipalities WHERE id = $1', [
          id,
        ]);
        ...
        res.json({
          ...
          message:
            force === 'true'
              ? 'Município excluído com sucesso (forçado)'
...
router.get('/audit', requireSupervisor, async (req, res) => {
  try {
    ...
    for (const table of tables) {
      try {
        ...
        if (exists.exists) {
          ...
          auditResults.database.tables[table].record_count = parseInt(
            count.count
          );
        }
      } catch (error) {
    ...
    try {
      ...
      auditResults.errors.push(
        `Erro ao verificar relacionamentos: ${error.message}`
      );
    }
    ...
    try {
      ...
      const locals = await getRows(
        'SELECT id, name, sector_id, municipality_id FROM locals'
      );
      auditResults.database.data_integrity.locals = locals;

      // Verificar se há patrimônios (usando colunas corretas)
      const patrimonios = await getRows(
        'SELECT id, numero_patrimonio, descricao, tipo FROM patrimonios LIMIT 10'
      );
      auditResults.database.data_integrity.patrimonios = patrimonios;

      // Verificar se há usuários
      const users = await getRows(
        'SELECT id, name, email, role, municipality_id FROM users'
      );
      ...
      try {
        const imoveis = await getRows(
          'SELECT id, numero_imovel, descricao, tipo FROM imoveis LIMIT 10'
        );
        auditResults.database.data_integrity.imoveis = imoveis;
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar imóveis: ${error.message}`
        );
      }

      // Verificar se há tarefas de manutenção
      try {
        const manutencao = await getRows(
          'SELECT id, descricao, tipo, status FROM manutencao_tasks LIMIT 10'
        );
        auditResults.database.data_integrity.manutencao = manutencao;
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar manutenção: ${error.message}`
        );
      }
      ...
      try {
        const transfers = await getRows(
          'SELECT id, patrimonio_id, origem_id, destino_id FROM transfers LIMIT 10'
        );
        auditResults.database.data_integrity.transfers = transfers;
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar transferências: ${error.message}`
        );
      }
      ...
      try {
        const inventories = await getRows(
          'SELECT id, nome, descricao, status FROM inventories LIMIT 10'
        );
        auditResults.database.data_integrity.inventories = inventories;
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar inventários: ${error.message}`
        );
      }

      // Verificar se há templates de relatório
      try {
        const reportTemplates = await getRows(
          'SELECT id, nome, tipo FROM report_templates LIMIT 10'
        );
        auditResults.database.data_integrity.report_templates = reportTemplates;
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar templates de relatório: ${error.message}`
        );
      }

      // Verificar se há templates de etiqueta
      try {
        const labelTemplates = await getRows(
          'SELECT id, nome, tipo FROM label_templates LIMIT 10'
        );
        auditResults.database.data_integrity.label_templates = labelTemplates;
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar templates de etiqueta: ${error.message}`
        );
      }
    } catch (error) {
      auditResults.errors.push(
        `Erro ao verificar integridade dos dados: ${error.message}`
      );
    }
    ...
    try {
      ...
      if (invalidLocals.length > 0) {
        auditResults.errors.push(
          `Encontrados ${invalidLocals.length} locais com setor inválido`
        );
      ...
      const hasLocalObjeto = patrimonioColumns.some(
        col => col.column_name === 'local_objeto'
      );
      const hasSetorResponsavel = patrimonioColumns.some(
        col => col.column_name === 'setor_responsavel'
      );
      ...
      if (hasLocalObjeto) {
        try {
          ...
          if (invalidPatrimonios.length > 0) {
            auditResults.errors.push(
              `Encontrados ${invalidPatrimonios.length} patrimônios com local inválido`
            );
            auditResults.database.data_integrity.invalid_patrimonios =
              invalidPatrimonios;
          }
        } catch (error) {
          auditResults.warnings.push(
            `Não foi possível verificar patrimônios com local inválido: ${error.message}`
          );
        }
      } else {
        auditResults.warnings.push(
          'Coluna local_objeto não existe na tabela patrimonios'
        );
      }
      ...
      if (hasSetorResponsavel) {
        try {
          ...
          if (invalidPatrimoniosSector.length > 0) {
            auditResults.errors.push(
              `Encontrados ${invalidPatrimoniosSector.length} patrimônios com setor inválido`
            );
            auditResults.database.data_integrity.invalid_patrimonios_sector =
              invalidPatrimoniosSector;
          }
        } catch (error) {
          auditResults.warnings.push(
            `Não foi possível verificar patrimônios com setor inválido: ${error.message}`
          );
        }
      } else {
        auditResults.warnings.push(
          'Coluna setor_responsavel não existe na tabela patrimonios'
        );
      }
      ...
      try {
        ...
        if (invalidTransfers.length > 0) {
          auditResults.errors.push(
            `Encontradas ${invalidTransfers.length} transferências com referências inválidas`
          );
          auditResults.database.data_integrity.invalid_transfers =
            invalidTransfers;
        }
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar integridade de transferências: ${error.message}`
        );
      }
      ...
      try {
        ...
        if (invalidManutencao.length > 0) {
          auditResults.errors.push(
            `Encontradas ${invalidManutencao.length} tarefas de manutenção com referências inválidas`
          );
          auditResults.database.data_integrity.invalid_manutencao =
            invalidManutencao;
        }
      } catch (error) {
        auditResults.warnings.push(
          `Não foi possível verificar integridade de manutenção: ${error.message}`
        );
      }
    } catch (error) {
      auditResults.errors.push(
        `Erro ao verificar problemas de integridade: ${error.message}`
      );
    }
    ...
    try {
      ...
      const usersResponse = await getRows(
    ...
    try {
      ...
      const sectorsResponse = await getRows(
        'SELECT id, name, municipality_id FROM sectors LIMIT 5'
      );
    ...
    try {
      ...
      const patrimoniosResponse = await getRows(
        'SELECT id, numero_patrimonio, descricao, tipo FROM patrimonios LIMIT 5'
      );
    ...
    try {
      ...
      auditResults.api_endpoints.municipalities.sample_data =
    ...
    try {
      ...
      auditResults.api_endpoints.activity_logs.sample_data =
    ...
    try {
      ...
      auditResults.api_endpoints.uploads.error =
        'Tabela uploads não existe - funcionalidade opcional';
    }
    ...
    try {
      ...
      if (localSample) {
        if (!localSample.sectorId) {
          fieldMappingIssues.push(
            'Campo sectorId não está sendo mapeado corretamente em locals'
          );
        }
        if (!localSample.municipalityId) {
          fieldMappingIssues.push(
            'Campo municipalityId não está sendo mapeado corretamente em locals'
          );
        }
      }
      ...
      auditResults.errors.push(
        `Erro ao verificar mapeamento de campos: ${error.message}`
      );
    }
    ...
    try {
      ...
      auditResults.errors.push(
        `Erro ao verificar integração com frontend: ${error.message}`
      );
    }
    ...
    try {
      if (auditResults.database.tables.activity_logs?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela activity_logs para registrar atividades do sistema'
        );
      }

      if (auditResults.database.data_integrity.municipalities?.length === 0) {
        auditResults.recommendations.push(
          'Adicionar pelo menos um município ao sistema'
        );
      }

      if (auditResults.database.data_integrity.sectors?.length === 0) {
        auditResults.recommendations.push(
          'Adicionar pelo menos um setor ao sistema'
        );
      }

      if (auditResults.database.data_integrity.users?.length === 0) {
        auditResults.recommendations.push(
          'Adicionar pelo menos um usuário ao sistema'
        );
      }
      ...
      if (auditResults.database.data_integrity.invalid_locals?.length > 0) {
        auditResults.recommendations.push(
          'Corrigir locais com setores inválidos'
        );
      }

      if (
        auditResults.database.data_integrity.invalid_patrimonios?.length > 0
      ) {
        auditResults.recommendations.push(
          'Corrigir patrimônios com locais inválidos'
        );
      }
      ...
      if (
        ...
        auditResults.recommendations.push(
          'Corrigir patrimônios com setores inválidos'
        );
      }

      if (auditResults.database.data_integrity.invalid_transfers?.length > 0) {
        auditResults.recommendations.push(
          'Corrigir transferências com referências inválidas'
        );
      }

      if (auditResults.database.data_integrity.invalid_manutencao?.length > 0) {
        auditResults.recommendations.push(
          'Corrigir tarefas de manutenção com referências inválidas'
        );
      }
      ...
      if (auditResults.database.tables.imoveis?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela imoveis para gerenciamento de imóveis'
        );
      }

      if (auditResults.database.tables.manutencao_tasks?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela manutencao_tasks para gerenciamento de manutenção'
        );
      }

      if (auditResults.database.tables.transfers?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela transfers para gerenciamento de transferências'
        );
      }

      if (auditResults.database.tables.inventories?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela inventories para gerenciamento de inventários'
        );
      }

      if (auditResults.database.tables.report_templates?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela report_templates para templates de relatórios'
        );
      }

      if (auditResults.database.tables.label_templates?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela label_templates para templates de etiquetas'
        );
      }
      ...
      if (auditResults.database.tables.form_fields?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela form_fields para campos personalizáveis'
        );
      }

      if (auditResults.database.tables.excel_csv_templates?.exists === false) {
        auditResults.recommendations.push(
          'Criar tabela excel_csv_templates para templates de exportação'
        );
      }

      if (
        auditResults.database.tables.customization_settings?.exists === false
      ) {
        auditResults.recommendations.push(
          'Criar tabela customization_settings para configurações do sistema'
        );
      }
      ...
      if (auditResults.database.data_integrity.report_templates?.length === 0) {
        auditResults.recommendations.push(
          'Criar templates de relatório de exemplo'
        );
      }

      if (auditResults.database.data_integrity.label_templates?.length === 0) {
        auditResults.recommendations.push(
          'Criar templates de etiqueta de exemplo'
        );
      }
    } catch (error) {
    ...
    auditResults.summary = {
      ...
      system_status:
        auditResults.errors.length === 0 ? 'HEALTHY' : 'NEEDS_ATTENTION',
    };

    console.log('✅ Auditoria concluída!');
    console.log(
      `📊 Resumo: ${auditResults.summary.total_errors} erros, ${auditResults.summary.total_warnings} avisos, ${auditResults.summary.total_recommendations} recomendações`
    );
...
router.post('/test-create', requireSupervisor, async (req, res) => {
  try {
    ...
    try {
      ...
      `,
        [
          'Local de Teste - Auditoria',
          testSector.rows[0].id,
      ...
      await query('DELETE FROM patrimonios WHERE numero_patrimonio = $1', [
        'TEST-001',
      ]);
      await query('DELETE FROM locals WHERE name = $1', [
        'Local de Teste - Auditoria',
      ]);
      await query('DELETE FROM sectors WHERE name = $1', [
        'Setor de Teste - Auditoria',
      ]);
...
router.post('/test-menu-navigation', requireSupervisor, async (req, res) => {
  try {
    console.log(
      '🧭 Iniciando testes de navegação e funcionalidades do menu...'
    );
    ...
    try {
      testResults.tests.bens_patrimonios = await testBensPatrimoniosFeatures(
        req.user
      );
      testResults.summary.total_tests +=
        testResults.tests.bens_patrimonios.total_operations;
      ...
      if (testResults.tests.bens_patrimonios.errors) {
        testResults.summary.errors.push(
          ...testResults.tests.bens_patrimonios.errors
        );
      }
    } catch (error) {
      testResults.summary.errors.push(
        `Erro no teste de bens/patrimônios: ${error.message}`
      );
    ...
    try {
      ...
      testResults.summary.total_tests +=
        testResults.tests.imoveis.total_operations;
      ...
      testResults.summary.errors.push(
        `Erro no teste de imóveis: ${error.message}`
      );
      testResults.summary.failed += 1;
    }
    ...
    try {
      ...
      testResults.summary.total_tests +=
        testResults.tests.ferramentas.total_operations;
      ...
      if (testResults.tests.ferramentas.errors) {
        testResults.summary.errors.push(
          ...testResults.tests.ferramentas.errors
        );
      }
    } catch (error) {
      testResults.summary.errors.push(
        `Erro no teste de ferramentas: ${error.message}`
      );
      testResults.summary.failed += 1;
    }
    ...
    try {
      testResults.tests.administracao = await testAdministracaoFeatures(
        req.user
      );
      testResults.summary.total_tests +=
        testResults.tests.administracao.total_operations;
      ...
      if (testResults.tests.administracao.errors) {
        testResults.summary.errors.push(
          ...testResults.tests.administracao.errors
        );
      }
    } catch (error) {
      testResults.summary.errors.push(
        `Erro no teste de administração: ${error.message}`
      );
      testResults.summary.failed += 1;
    }
    ...
    if (req.user.role === 'superuser') {
      ...
      try {
        ...
        testResults.summary.total_tests +=
          testResults.tests.superuser.total_operations;
        ...
        if (testResults.tests.superuser.errors) {
          testResults.summary.errors.push(
            ...testResults.tests.superuser.errors
          );
        }
      } catch (error) {
        testResults.summary.errors.push(
          `Erro no teste de superusuário: ${error.message}`
        );
        testResults.summary.failed += 1;
      }
    }
    ...
    console.log(
      `✅ Testes de navegação concluídos! ${testResults.summary.passed}/${testResults.summary.total_tests} funcionalidades testadas com sucesso`
    );
...
router.post('/test-crud-operations', requireSupervisor, async (req, res) => {
  try {
    ...
    try {
      ...
      testResults.summary.total_tests +=
        testResults.tests.sectors.total_operations;
      ...
      testResults.summary.errors.push(
        `Erro no teste de setores: ${error.message}`
      );
      testResults.summary.failed += 1;
    }
    ...
    try {
      ...
      testResults.summary.total_tests +=
        testResults.tests.locals.total_operations;
      ...
      testResults.summary.errors.push(
        `Erro no teste de locais: ${error.message}`
      );
      testResults.summary.failed += 1;
    }
    ...
    try {
      ...
      testResults.summary.total_tests +=
        testResults.tests.patrimonios.total_operations;
      ...
      if (testResults.tests.patrimonios.errors) {
        testResults.summary.errors.push(
          ...testResults.tests.patrimonios.errors
        );
      }
    } catch (error) {
      testResults.summary.errors.push(
        `Erro no teste de patrimônios: ${error.message}`
      );
      testResults.summary.failed += 1;
    }
    ...
    try {
      ...
      testResults.summary.total_tests +=
        testResults.tests.manutencao.total_operations;
      ...
      testResults.summary.errors.push(
        `Erro no teste de manutenção: ${error.message}`
      );
      testResults.summary.failed += 1;
    }

    console.log(
      `✅ Testes CRUD concluídos! ${testResults.summary.passed}/${testResults.summary.total_tests} operações bem-sucedidas`
    );
...
router.get('/table/:tableName', requireSupervisor, async (req, res) => {
  try {
    ...
    if (!exists.exists) {
      return res
        .status(404)
        .json({ error: `Tabela ${tableName} não encontrada` });
    }
    ...
    console.error(
      `❌ Erro ao verificar tabela ${req.params.tableName}:`,
      error
    );
    res.status(500).json({
      error: 'Erro interno do servidor',
...
async function testSectorsCRUD(user) {
  ...
  try {
    ...
    const readResult = await query('SELECT * FROM sectors WHERE id = $1', [
      testSectorId,
    ]);
    if (readResult.rows.length > 0) {
      ...
      results.operations.read = {
        status: 'FAILED',
        error: 'Setor não encontrado após criação',
      };
      results.failed++;
    }
    ...
    if (deleteResult.rows.length > 0) {
      results.operations.delete = {
        status: 'SUCCESS',
        data: { deleted_id: deleteResult.rows[0].id },
      };
      results.passed++;
    } else {
      results.operations.delete = {
        status: 'FAILED',
        error: 'Falha ao deletar setor',
      };
      results.failed++;
    }
  } catch (error) {
...
async function testLocalsCRUD(user) {
  ...
  try {
    ...
    const readResult = await query('SELECT * FROM locals WHERE id = $1', [
      testLocalId,
    ]);
    if (readResult.rows.length > 0) {
      results.operations.read = { status: 'SUCCESS', data: readResult.rows[0] };
      ...
      results.operations.read = {
        status: 'FAILED',
        error: 'Local não encontrado após criação',
      };
      results.failed++;
    }
    ...
    if (updateResult.rows.length > 0) {
      ...
      results.operations.update = {
        status: 'FAILED',
        error: 'Falha ao atualizar local',
      };
      results.failed++;
    }
    ...
    const deleteResult = await query(
      'DELETE FROM locals WHERE id = $1 RETURNING id',
      [testLocalId]
    );
    if (deleteResult.rows.length > 0) {
      results.operations.delete = {
...
async function testPatrimoniosCRUD(user) {
  ...
  try {
    ...
    const readResult = await query('SELECT * FROM patrimonios WHERE id = $1', [
      testPatrimonioId,
    ]);
    if (readResult.rows.length > 0) {
      results.operations.read = { status: 'SUCCESS', data: readResult.rows[0] };
      ...
      results.operations.read = {
        status: 'FAILED',
        error: 'Patrimônio não encontrado após criação',
      };
      results.failed++;
    }
    ...
    if (updateResult.rows.length > 0) {
      ...
      results.operations.update = {
        status: 'FAILED',
        error: 'Falha ao atualizar patrimônio',
      };
      results.failed++;
    }
    ...
    if (deleteResult.rows.length > 0) {
      ...
      results.operations.delete = {
        status: 'FAILED',
        error: 'Falha ao deletar patrimônio',
      };
      results.failed++;
    }
  } catch (error) {
...
async function testImoveisCRUD(user) {
  ...
  try {
    ...
    const readResult = await query('SELECT * FROM imoveis WHERE id = $1', [
      testImovelId,
    ]);
    if (readResult.rows.length > 0) {
      results.operations.read = { status: 'SUCCESS', data: readResult.rows[0] };
      ...
      results.operations.read = {
        status: 'FAILED',
        error: 'Imóvel não encontrado após criação',
      };
      results.failed++;
    }
...
async function testManutencaoCRUD(user) {
  ...
  try {
    ...
    const readResult = await query(
      'SELECT * FROM manutencao_tasks WHERE id = $1',
      [testManutencaoId]
    );
    if (readResult.rows.length > 0) {
      results.operations.read = { status: 'SUCCESS', data: readResult.rows[0] };
      ...
      results.operations.read = {
        status: 'FAILED',
        error: 'Manutenção não encontrada após criação',
      };
      results.failed++;
    }
    ...
    if (updateResult.rows.length > 0) {
      ...
      results.operations.update = {
        status: 'FAILED',
        error: 'Falha ao atualizar manutenção',
      };
      results.failed++;
    }
    ...
    const deleteResult = await query(
      'DELETE FROM manutencao_tasks WHERE id = $1 RETURNING id',
      [testManutencaoId]
    );
    if (deleteResult.rows.length > 0) {
      results.operations.delete = {
      ...
      results.operations.delete = {
        status: 'FAILED',
        error: 'Falha ao deletar manutenção',
      };
      results.failed++;
    }
    ...
    if (testPatrimonioId) {
      try {
        await query('DELETE FROM patrimonios WHERE id = $1', [
          testPatrimonioId,
        ]);
      } catch (cleanupError) {
        // Ignorar erro de limpeza
...
async function testBensPatrimoniosFeatures(user) {
  ...
  try {
    ...
    const viewResult = await query('SELECT * FROM patrimonios WHERE id = $1', [
      testPatrimonioId,
    ]);
    ...
    console.log('  🧹 Limpando dados de teste...');
    await query('DELETE FROM transfers WHERE patrimonio_id = $1', [
      testPatrimonioId,
    ]);
    await query('DELETE FROM patrimonios WHERE id = $1', [testPatrimonioId]);
...
async function testImoveisFeatures(user) {
  ...
  try {
    ...
    const viewResult = await query('SELECT * FROM imoveis WHERE id = $1', [
      testImovelId,
    ]);
    if (viewResult.rows.length > 0) {
...
async function testFerramentasFeatures(user) {
  ...
  try {
    ...
    const reportTemplatesResult = await query(
    ...
    `,
      [
        'Template Teste Navegação',
        'patrimonios',
        'Template para teste de navegação',
        JSON.stringify({ colunas: ['numero_patrimonio', 'descricao'] }),
        user.id,
      ]
    );
    ...
    const labelTemplatesResult = await query(
      'SELECT COUNT(*) as total FROM label_templates'
    );
    ...
    `,
      [
        'Etiqueta Teste Navegação',
        'Etiqueta para teste de navegação',
        'padrao',
        JSON.stringify({ largura: 100, altura: 50 }),
        user.id,
      ]
    );
    ...
    const exportResult = await query(
      'SELECT COUNT(*) as total FROM excel_csv_templates'
    );
    ...
    `,
      [
        'Export Teste Navegação',
        'patrimonios',
        'Template de exportação para teste',
        JSON.stringify(['numero_patrimonio', 'descricao']),
        'xlsx',
        user.id,
      ]
    );
    ...
    const mapResult = await query(
      'SELECT COUNT(*) as total FROM imoveis WHERE latitude IS NOT NULL AND longitude IS NOT NULL'
    );
    ...
    console.log('  🧹 Limpando dados de teste...');
    await query('DELETE FROM excel_csv_templates WHERE id = $1', [
      testExportId,
    ]);
...
async function testAdministracaoFeatures(user) {
  ...
  try {
    ...
    const settingsResult = await query(
      'SELECT COUNT(*) as total FROM customization_settings'
    );
...
async function testSystemSetup(user) {
  ...
  try {
    ...
    const userTest = await query(
      'SELECT role, municipality_id FROM users WHERE id = $1',
      [user.id]
    );
    if (userTest.rows.length > 0) {
      results.operations.user_permissions = {
        ...
        data: {
          ...
          municipality_id: userTest.rows[0].municipality_id,
        },
      };
      results.passed++;
    } else {
...
async function testMunicipalitySetup(user) {
  ...
  try {
    ...
    `,
      [
        `Supervisor Teste ${timestamp}`,
        `supervisor${timestamp}@teste.com`,
        'senha123',
        'supervisor',
        testMunicipalityId,
      ]
    ...
    `,
      [
        `Usuário Teste ${timestamp}`,
        `usuario${timestamp}@teste.com`,
        'senha123',
        'user',
        testMunicipalityId,
      ]
    ...
    results.errors.push(
      `Erro na criação de município e usuários: ${error.message}`
    );
    results.failed++;
  }
  ...
  try {
    ...
    `,
      [
        ...
        'Setor administrativo principal',
        testMunicipalityId,
        user.id,
      ]
    );
    ...
    `,
      [
        'Sala de Reuniões',
        'Sala para reuniões e treinamentos',
        childSectorId,
        testMunicipalityId,
        user.id,
      ]
...
async function testPatrimonyManagement(user) {
  ...
  try {
    ...
    `,
      ['Computador Desktop Completo - Editado', 2700.0, basicPatrimonyId]
    );
    ...
    results.errors.push(
      `Erro no gerenciamento de patrimônios: ${error.message}`
    );
    results.failed++;
  }
  ...
  try {
    ...
    `,
      [
        'IMOVEL-002',
        'Galpão Comercial',
        'Av. Industrial, 456 - Zona Industrial',
        500.0,
        'Comercial',
        300000.0,
        '2021-08-20',
    ...
    const editResult = await query(
      ...
      RETURNING id, descricao, valor_aquisicao
    `,
      ['Casa Residencial Municipal - Editada', 160000.0, residentialId]
    );
  ...
  try {
    ...
    `,
      [
        ...
        null,
        testDestLocalId,
        'Transferência para teste do sistema',
        user.id,
      ]
    );
    ...
    results.operations.transfer_history = {
      status: 'SUCCESS',
      data: {
        total_transfers: parseInt(historyResult.rows[0].total_transfers),
      },
    };
    results.passed++;
    ...
    results.errors.push(
      `Erro nas operações de transferência: ${error.message}`
    );
    results.failed++;
  }
  ...
  try {
    ...
    `,
      [
        testPatrimonyId,
        'Preventiva',
        'Limpeza e calibração da impressora',
        '2024-12-15',
        'pendente',
        'baixa',
        150.0,
  ...
  try {
    ...
    `,
      [
        ...
        'Template para relatório completo',
        'patrimonios',
        JSON.stringify({
          columns: [
            'numero_patrimonio',
    ...
    `,
      [
        'Etiqueta Completa de Patrimônio',
        'Template para etiquetas completas',
        JSON.stringify({
    ...
    results.operations.list_templates = {
      status: 'SUCCESS',
      data: {
        total_templates: parseInt(listTemplatesResult.rows[0].total_templates),
      },
    };
    results.passed++;
  ...
  try {
    ...
    try {
      ...
      testResults.summary.total_operations +=
        testResults.tests.setup.total_operations;
    ...
    try {
      testResults.tests.municipality_setup = await testMunicipalitySetup(
        req.user
      );
      testResults.summary.total_operations +=
        testResults.tests.municipality_setup.total_operations;
      ...
      testResults.errors.push(
        `Erro crítico na criação de município: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      testResults.tests.organizational_structure =
        await testOrganizationalStructure(req.user);
      testResults.summary.total_operations +=
        testResults.tests.organizational_structure.total_operations;
      testResults.summary.passed +=
        testResults.tests.organizational_structure.passed;
      testResults.summary.failed +=
        testResults.tests.organizational_structure.failed;
      if (testResults.tests.organizational_structure.errors) {
        testResults.errors.push(
          ...testResults.tests.organizational_structure.errors
        );
      }
    } catch (error) {
      testResults.errors.push(
        `Erro crítico na estrutura organizacional: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      testResults.tests.patrimony_management = await testPatrimonyManagement(
        req.user
      );
      testResults.summary.total_operations +=
        testResults.tests.patrimony_management.total_operations;
      testResults.summary.passed +=
        testResults.tests.patrimony_management.passed;
      testResults.summary.failed +=
        testResults.tests.patrimony_management.failed;
      if (testResults.tests.patrimony_management.errors) {
        testResults.errors.push(
          ...testResults.tests.patrimony_management.errors
        );
      }
    } catch (error) {
      testResults.errors.push(
        `Erro crítico no cadastro de patrimônios: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      testResults.tests.real_estate_management = await testRealEstateManagement(
        req.user
      );
      testResults.summary.total_operations +=
        testResults.tests.real_estate_management.total_operations;
      testResults.summary.passed +=
        testResults.tests.real_estate_management.passed;
      testResults.summary.failed +=
        testResults.tests.real_estate_management.failed;
      if (testResults.tests.real_estate_management.errors) {
        testResults.errors.push(
          ...testResults.tests.real_estate_management.errors
        );
      }
    } catch (error) {
      testResults.errors.push(
        `Erro crítico na gestão de imóveis: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      testResults.tests.transfer_operations = await testTransferOperations(
        req.user
      );
      testResults.summary.total_operations +=
        testResults.tests.transfer_operations.total_operations;
      testResults.summary.passed +=
        testResults.tests.transfer_operations.passed;
      testResults.summary.failed +=
        testResults.tests.transfer_operations.failed;
      if (testResults.tests.transfer_operations.errors) {
        testResults.errors.push(
          ...testResults.tests.transfer_operations.errors
        );
      }
    } catch (error) {
      testResults.errors.push(
        `Erro crítico nas transferências: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      testResults.tests.maintenance_system = await testMaintenanceSystem(
        req.user
      );
      testResults.summary.total_operations +=
        testResults.tests.maintenance_system.total_operations;
      ...
      testResults.errors.push(
        `Erro crítico no sistema de manutenção: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      testResults.tests.reports_and_labels = await testReportsAndLabels(
        req.user
      );
      testResults.summary.total_operations +=
        testResults.tests.reports_and_labels.total_operations;
      ...
      testResults.errors.push(
        `Erro crítico em relatórios e etiquetas: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      testResults.tests.dashboard_analytics = await testDashboardAnalytics(
        req.user
      );
      testResults.summary.total_operations +=
        testResults.tests.dashboard_analytics.total_operations;
      testResults.summary.passed +=
        testResults.tests.dashboard_analytics.passed;
      testResults.summary.failed +=
        testResults.tests.dashboard_analytics.failed;
      if (testResults.tests.dashboard_analytics.errors) {
        testResults.errors.push(
          ...testResults.tests.dashboard_analytics.errors
        );
      }
    } catch (error) {
    ...
    try {
      ...
      testResults.summary.total_operations +=
        testResults.tests.export_backup.total_operations;
      ...
      testResults.errors.push(
        `Erro crítico na exportação/backup: ${error.message}`
      );
      testResults.summary.critical_errors++;
    }
    ...
    try {
      ...
      testResults.summary.total_operations +=
        testResults.tests.cleanup.total_operations;
    ...
    console.log(
      `✅ Teste completo finalizado em ${Math.round(testResults.duration / 1000)}s`
    );
    console.log(`📊 Status: ${testResults.summary.system_status}`);
    console.log(
      `✅ Sucessos: ${testResults.summary.passed}/${testResults.summary.total_operations}`
    );
...
router.post('/fix-common-issues', requireSupervisor, async (req, res) => {
  try {
    ...
    try {
          ...
          try {
            ...
            switch (column) {
              case 'historico_movimentacao':
                alterQuery =
                  "ALTER TABLE patrimonios ADD COLUMN IF NOT EXISTS historico_movimentacao jsonb DEFAULT '[]'::jsonb";
                break;
              case 'local_objeto':
                alterQuery =
                  'ALTER TABLE patrimonios ADD COLUMN IF NOT EXISTS local_objeto character varying';
                break;
              case 'setor_responsavel':
                alterQuery =
                  'ALTER TABLE patrimonios ADD COLUMN IF NOT EXISTS setor_responsavel character varying';
                break;
              case 'cor':
                alterQuery =
              ...
              case 'status':
                alterQuery =
                  "ALTER TABLE patrimonios ADD COLUMN IF NOT EXISTS status character varying DEFAULT 'ativo'";
                break;
            }
            ...
            fixResults.summary.errors.push(
              `Erro ao adicionar coluna ${column}: ${error.message}`
            );
      ...
      fixResults.summary.errors.push(
        `Erro ao verificar estrutura de patrimonios: ${error.message}`
      );
      fixResults.summary.failed++;
    }
    ...
    try {
      // Verificar se há municípios
      const municipalitiesCount = await getRow(
      ...
      fixResults.summary.errors.push(
        `Erro ao criar município de exemplo: ${error.message}`
      );
      fixResults.summary.failed++;
    }
    ...
    try {
      ...
      fixResults.summary.errors.push(
        `Erro ao criar setores de exemplo: ${error.message}`
      );
      fixResults.summary.failed++;
    }
    ...
    try {
      ...
      fixResults.summary.errors.push(
        `Erro ao criar locais de exemplo: ${error.message}`
      );
      fixResults.summary.failed++;
    }
    ...
    try {
      ...
      fixResults.summary.errors.push(
        `Erro ao criar patrimônios de exemplo: ${error.message}`
      );
      fixResults.summary.failed++;
    }
    ...
    try {
      ...
      if (invalidLocals.length > 0) {
        ...
        for (const local of invalidLocals) {
          ...
          if (sector) {
            await query('UPDATE locals SET sector_id = $1 WHERE id = $2', [
              sector.id,
              local.id,
            ]);
          }
      ...
      fixResults.summary.errors.push(
        `Erro ao corrigir integridade de locais: ${error.message}`
      );
      fixResults.summary.failed++;
    }
    ...
    fixResults.summary.system_status =
      fixResults.summary.failed === 0 ? 'HEALTHY' : 'NEEDS_ATTENTION';

    console.log('✅ Correção de problemas concluída!');
    console.log(
      `📊 Resumo: ${fixResults.summary.successful} correções bem-sucedidas, ${fixResults.summary.failed} falhas`
    );
  ...
  async (req, res) => {
    ...
    `,
      ...
      console.log(
        ...
        patrimonios.map(p => p.numero_patrimonio)
      );
