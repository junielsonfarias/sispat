...
router.post('/generate', async (req, res) => {
  try {
    const { type, parameters = {}, priority = 2 } = req.body;
...
router.get('/jobs/:jobId/download', async (req, res) => {
  try {
    ...
    if (format === 'csv' && result.csvData) {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="report-${jobId}.json"`
      );
      res.json(result);
    } else {
      res.status(400).json({
  ...
  const allTypes = [
    {
      ...
      description:
        'Relatório consolidado de patrimônios por setor, tipo e situação',
      ...
      parameters: [
        {
          ...
          type: 'string',
          optional: true,
          description: 'ID do setor específico',
        },
        {
          name: 'dateFrom',
          type: 'date',
    ...
    },
    {
      ...
      description:
        'Cálculo de depreciação acumulada e valor atual dos patrimônios',
    ...
    },
    {
      ...
      description:
        'Relatório detalhado de todas as transferências em um período',
      ...
      parameters: [
        {
          ...
          type: 'string',
          required: true,
          description: 'Tipo de dados (patrimonios, users)',
        },
        {
          name: 'format',
          type: 'string',
      ...
      parameters: [
        {
          ...
          type: 'object',
          required: true,
          description: 'Configurações do relatório',
        },
      ],
    },
  ];
