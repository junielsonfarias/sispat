...
import { authenticateToken, requireSupervisor, requireUser } from '../middleware/auth.js'
import {
...
function validateAndLimitImageSize(base64String, maxSizeInMB = 5) {
  try {
    ...
    if (sizeInMB > maxSizeInMB) {
      console.log(`⚠️ Imagem muito grande (${sizeInMB.toFixed(2)} MB). Máximo permitido: ${maxSizeInMB} MB`)
...
router.get('/test-table', async (req, res) => {
  try {
    ...
    if (!tableExists.exists) {
      return res.status(404).json({ error: 'Tabela patrimonios não encontrada' })
    }
    ...
    // Get total records
    const totalRecords = await getRow('SELECT COUNT(*) as count FROM patrimonios')
...
router.get('/test-public-query', async (req, res) => {
  try {
    ...
    const basicQuery = 'SELECT COUNT(*) as count FROM patrimonios WHERE deleted_at IS NULL'
    ...
    if (municipalityId) {
      ...
      `
      const municipalityResult = await getRow(municipalityQuery, [municipalityId])
...
router.get('/test-simple', async (req, res) => {
  try {
    ...
    console.error('❌ Stack trace:', error.stack)
    res.status(500).json({ error: 'Erro na query simples', details: error.message })
  }
})
...
router.get('/test-main-query', async (req, res) => {
  try {
    ...
    console.error('❌ Stack trace:', error.stack)
    res.status(500).json({ error: 'Erro na query principal', details: error.message })
  }
})
...
router.get('/test-pagination', async (req, res) => {
  try {
    ...
    // Count total
    const countQuery = 'SELECT COUNT(*) as total FROM patrimonios WHERE deleted_at IS NULL'
    ...
    console.error('❌ Stack trace:', error.stack)
    res.status(500).json({ error: 'Erro na paginação', details: error.message })
  }
})
...
router.get('/test-build-pagination', async (req, res) => {
  try {
    ...
    console.error('❌ Stack trace:', error.stack)
    res.status(500).json({ error: 'Erro na buildPaginatedQuery', details: error.message })
  }
})
...
router.get('/test-main-with-pagination', async (req, res) => {
  try {
    ...
    console.error('❌ Stack trace:', error.stack)
    res.status(500).json({ error: 'Erro na query principal com buildPaginatedQuery', details: error.message })
  }
})
...
router.get('/test-execute-pagination', async (req, res) => {
  try {
    ...
    console.error('❌ Stack trace:', error.stack)
    res.status(500).json({ error: 'Erro na executePaginatedQuery', details: error.message })
  }
})
...
router.get('/count', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('👤 Usuário:', req.user.name, 'Role:', req.user.role, 'Municipality:', req.user.municipality_id)
    ...
    if (req.user.role !== 'superuser') {
      ...
      console.log('🔍 Contando patrimônios do município:', req.user.municipality_id)
    } else {
...
router.get('/test-structure', async (req, res) => {
  try {
    ...
    const missingColumns = [
      'cor', 'quantidade', 'fotos', 'documentos', 
      'metodo_depreciacao', 'vida_util_anos', 'valor_residual'
...
router.post('/test-create', async (req, res) => {
  try {
    ...
    // Validation
    if (!numero_patrimonio || !descricao_bem || !tipo || !data_aquisicao || !valor_aquisicao) {
      console.log('❌ Validação falhou - campos obrigatórios faltando')
      return res.status(400).json({ 
        error: 'Campos obrigatórios: número do patrimônio, descrição, tipo, data de aquisição, valor' 
      })
    }
    ...
    console.log('✅ Usuário encontrado para teste:', userCheck.name, 'ID:', testUserId)

    // Check if patrimonio number already exists in this municipality
    console.log('🔍 Verificando se patrimônio já existe:', numero_patrimonio, 'no município:', municipalityId)
    ...
    console.log('✅ Patrimônio criado com sucesso:', newPatrimonio.id, newPatrimonio.numero_patrimonio)
...
router.get('/public/:numeroPatrimonio', async (req, res) => {
  try {
    ...
    console.log(`🌐 CONSULTA PÚBLICA - Buscando patrimônio: ${numeroPatrimonio}`)
...
router.get('/debug/public-sync', async (req, res) => {
  try {
    ...
    const municipalities = await getRows('SELECT id, name, state FROM municipalities')
    ...
    for (const municipality of municipalities) {
      ...
      console.log(`📋 Município ${municipality.name}: ${patrimonios.length} patrimônios`)
    }
    ...
    res.json({
      ...
      totalPatrimonios: patrimoniosData.reduce((sum, data) => sum + data.count, 0)
    })
  } catch (error) {
...
router.get('/', patrimoniosCacheMiddleware, paginationMiddleware, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'superuser' && !req.user.municipality_id) {
      ...
      return res.status(400).json({ error: 'Usuário sem município definido' })
    }
    ...
    if (req.user.role === 'usuario' || req.user.role === 'visualizador') {
      ...
      if (userSectors.length > 0) {
        ...
        console.log('⚠️ Usuário sem setores atribuídos - sem acesso a patrimônios')
        return res.json({
          items: [],
          pagination: {
            hasMore: false,
    ...
    const finalQuery = patrimoniosQuery + ` ORDER BY p.numero_patrimonio LIMIT $${params.length + 1}`
    ...
    const result = {
      ...
      pagination: {
        ...
        nextCursor: patrimonios.length > 0 ? patrimonios[patrimonios.length - 1].numero_patrimonio : null,
        prevCursor: null,
        count: patrimonios.length,
        limit: limit
      }
    ...
    console.log('📋 Patrimônios:', result.items.map(p => ({ 
      id: p.id, 
    ...
    const processedPatrimonios = await Promise.all(result.items.map(async (patrimonio) => {
      try {
        console.log('🔄 Processando patrimônio:', patrimonio.numero_patrimonio)
        ...
        console.error(`Erro ao processar histórico do patrimônio ${patrimonio.id}:`, error)
        return {
          ...patrimonio,
          fotos: patrimonio.fotos ? JSON.parse(patrimonio.fotos) : [],
          documentos: patrimonio.documentos ? JSON.parse(patrimonio.documentos) : [],
    ...
    console.log('✅ Patrimônios processados:', processedItems.length, 'HasMore:', result.pagination.hasMore)
...
router.post('/', invalidatePatrimoniosCache, async (req, res) => {
  try {
    ...
    if (!numero_patrimonio || !descricao_bem || !tipo || !data_aquisicao || !valor_aquisicao) {
      console.log('❌ Validação falhou - campos obrigatórios faltando')
      return res.status(400).json({ 
        error: 'Campos obrigatórios: número do patrimônio, descrição, tipo, data de aquisição e valor' 
      })
    }
    ...
    if (local_objeto) {
      console.log('🔍 Buscando local:', local_objeto, 'no município:', municipalityId)
      ...
      const allLocals = await getRows(`
        ...
        ORDER BY name
      `, [municipalityId])
      console.log('📋 Todos os locais disponíveis:', allLocals.map(l => ({ id: l.id, name: l.name })))
      ...
      if (local) {
        ...
        if (similarLocal) {
          ...
          console.log('✅ Local encontrado por similaridade:', similarLocal.name, '→', localId)
        } else {
          console.log('❌ Local não encontrado mesmo com busca por similaridade')
        }
      }
    }

    if (setor_responsavel) {
      console.log('🔍 Buscando setor:', setor_responsavel, 'no município:', municipalityId)
      ...
      const allSectors = await getRows(`
        ...
        ORDER BY name
      `, [municipalityId])
      console.log('📋 Todos os setores disponíveis:', allSectors.map(s => ({ id: s.id, name: s.name })))
      ...
      if (sector) {
        ...
        if (similarSector) {
          ...
          console.log('✅ Setor encontrado por similaridade:', similarSector.name, '→', sectorId)
        } else {
          console.log('❌ Setor não encontrado mesmo com busca por similaridade')
        }
      }
    }
    ...
    const processedPatrimonio = {
      ...
      fotos: patrimonioWithCompleteData.fotos ? JSON.parse(patrimonioWithCompleteData.fotos) : [],
      documentos: patrimonioWithCompleteData.documentos ? JSON.parse(patrimonioWithCompleteData.documentos) : []
    }

    console.log('✅ Patrimônio retornado com dados completos:', processedPatrimonio)
    ...
    if (error.code === '23505') { // Unique constraint violation
      ...
      return res.status(400).json({ error: 'Número do patrimônio já existe neste município' })
    }
    
    // Verificar se é erro de JSON
    if (error.message && error.message.includes('JSON')) {
      console.error('❌ Erro de JSON nas fotos/documentos')
      return res.status(400).json({ error: 'Erro nos dados de fotos ou documentos' })
    }
    
    // Verificar se é erro de tamanho
    if (error.message && (error.message.includes('too long') || error.message.includes('value too long'))) {
      console.error('❌ Dados muito grandes')
      return res.status(400).json({ error: 'Dados muito grandes (fotos ou documentos). Tente sem fotos.' })
    }
    ...
    if (error.message && (error.message.includes('memory') || error.message.includes('timeout'))) {
      ...
      return res.status(400).json({ error: 'Dados muito pesados. Tente sem fotos.' })
    }
...
// Update patrimonio
router.put('/:id', requireUser, invalidatePatrimoniosCache, async (req, res) => {
  try {
    const { id } = req.params
    ...
    // Check if user has access to this patrimonio's municipality
    if (req.user.role !== 'superuser' && existingPatrimonio.municipality_id !== req.user.municipality_id) {
      return res.status(403).json({ error: 'Acesso negado' })
    }
    ...
    if (updateData.situacao_bem !== undefined) {
      ...
      console.log(`🔄 Mapeando situacao_bem → estado: ${updateData.situacao_bem}`)
    }
    ...
    if (updateData.forma_aquisicao !== undefined) {
      ...
      console.log(`🔄 Mapeando forma_aquisicao → fornecedor: ${updateData.forma_aquisicao}`)
    }
    ...
    if (updateData.numero_nota_fiscal !== undefined) {
      ...
      console.log(`🔄 Mapeando numero_nota_fiscal → nota_fiscal: ${updateData.numero_nota_fiscal}`)
    }
    ...
    if (updateData.descricao_bem !== undefined) {
      ...
      console.log(`🔄 Mapeando descricao_bem → descricao: ${updateData.descricao_bem}`)
    }

    const fieldsToUpdate = [
      'numero_patrimonio', 'descricao', 'tipo', 'marca', 'modelo', 'numero_serie',
      'data_aquisicao', 'valor_aquisicao', 'estado', 'fornecedor', 'nota_fiscal', 'status',
      'setor_responsavel', 'local_objeto', 'situacao_bem', 'cor', 'quantidade', 'forma_aquisicao',
      'metodo_depreciacao', 'vida_util_anos', 'valor_residual', 'fotos', 'documentos'
    ]
    ...
    Object.keys(processedUpdateData).forEach(field => {
      ...
      if (processedUpdateData[field] !== undefined) {
        ...
        console.log(`⚠️ Campo ${field} não encontrado nos dados de atualização`)
      }
    })
    ...
    if (updateData.descricao_bem || updateData.valor_aquisicao || updateData.estado) {
      const changes = []
    ...
    await query(
      ...
      [req.user.id, 'PATRIMONIO_UPDATE', 'patrimonios', id, existingPatrimonio.municipality_id]
    )
...
// Delete patrimonio
router.delete('/:id', requireSupervisor, invalidatePatrimoniosCache, async (req, res) => {
  try {
    const { id } = req.params
    ...
    await query('UPDATE patrimonios SET deleted_at = NOW() WHERE id = $1', [id])
...
router.get('/debug/check-data', async (req, res) => {
  try {
    ...
    // Check total count
    const totalCount = await getRow('SELECT COUNT(*) as total FROM patrimonios')
...
router.get('/debug/check-sectors-locals/:municipalityId', async (req, res) => {
  try {
    const { municipalityId } = req.params
    console.log('🔍 Verificando setores e locais para município:', municipalityId)
...
router.get('/debug/public/check-municipality/:municipalityName', async (req, res) => {
  try {
...
router.delete('/debug/public/delete-municipality/:municipalityName', async (req, res) => {
  try {
    const { municipalityName } = req.params
...
router.post('/:id/notes', requireSupervisor, invalidatePatrimoniosCache, async (req, res) => {
  try {
    console.log('📝 POST /:id/notes - Criando nova nota')
    console.log('👤 Usuário:', req.user.name, 'ID:', req.user.id, 'Role:', req.user.role)
    ...
    await query(
      'INSERT INTO activity_logs (user_id, action, table_name, record_id, municipality_id) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, 'PATRIMONIO_NOTE_ADDED', 'patrimonios', id, patrimonio.municipality_id]
    )
...
router.get('/debug/test-query/:sectorId', authenticateToken, async (req, res) => {
  try {
    ...
    const municipalityPatrimonios = await getRows(`
      ...
      ORDER BY numero_patrimonio
    `, [municipalityId])
    console.log('🧪 DEBUG - Patrimônios do município:', municipalityPatrimonios)
...
router.get('/generate-number/:sectorId', 
  authenticateToken, 
  ...
  try {
    ...
    console.log('🔢 Gerando número para setor:', sectorId, 'município:', municipalityId)
    ...
    if (!sector.codigo) {
      return res.status(400).json({ error: 'Setor não possui código definido' })
    }
    ...
    console.log('🔍 TODOS os patrimônios do município:', allMunicipalityPatrimonios.map(p => p.numero_patrimonio))
    
    // Agora filtrar apenas os que começam com o prefixo
    const existingPatrimonios = allMunicipalityPatrimonios.filter(p => 
      p.numero_patrimonio && p.numero_patrimonio.startsWith(prefix)
    )

    console.log('📊 Patrimônios existentes:', existingPatrimonios.length)
    console.log('📋 Números:', existingPatrimonios.map(p => p.numero_patrimonio))
    ...
    if (existingPatrimonios.length === 0) {
      console.log('📊 Nenhum patrimônio encontrado com este prefixo, começando do 1')
      maxSequence = 0
    } else {
      existingPatrimonios.forEach((p) => {
        ...
        console.log(`🔍 Analisando: ${p.numero_patrimonio} -> sequência: ${sequenceStr} -> número: ${sequenceNum}`)
