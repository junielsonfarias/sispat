...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (
      ...
      transfer.municipality_id !== req.user.municipality_id
    ) {
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        req.user.name,
        'TRANSFER_CREATE',
        `Transferência "${patrimonioNumero}" solicitada.`,
        municipalityId,
      ]
    );
...
router.put('/:id/status', requireUser, async (req, res) => {
  try {
    ...
    if (
      req.user.role !== 'superuser' &&
      existingTransfer.municipality_id !== req.user.municipality_id
    ) {
      return res.status(403).json({ error: 'Acesso negado' });
    }
    ...
    await query(
      ...
      [
        ...
        `Transferência "${updatedTransfer.patrimonio_numero}" ${status}.`,
        existingTransfer.municipality_id,
      ]
    );
