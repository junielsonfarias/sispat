...
router.get('/templates/:id', async (req, res) => {
  try {
    ...
    if (!template) {
      return res.status(404).json({ error: 'Template de relatório não encontrado' })
    }

    // Check if user has access to this template's municipality
    if (req.user.role !== 'superuser' && template.municipality_id !== req.user.municipality_id) {
...
router.post('/templates', requireSupervisor, async (req, res) => {
  try {
    ...
    const result = await query(`
      ...
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *
    `, [name, JSON.stringify(fields), JSON.stringify(filters), JSON.stringify(layout), municipalityId])
    ...
    await query(
      ...
      [req.user.id, req.user.name, 'REPORT_TEMPLATE_CREATE', `Template de relatório "${name}" criado.`, municipalityId]
    )
...
router.put('/templates/:id', requireSupervisor, async (req, res) => {
  try {
    ...
    // Check if user has access to this template's municipality
    if (req.user.role !== 'superuser' && existingTemplate.municipality_id !== req.user.municipality_id) {
      return res.status(403).json({ error: 'Acesso negado' })
    }
    ...
    await query(
      'INSERT INTO activity_logs (user_id, user_name, action, details, municipality_id) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, req.user.name, 'REPORT_TEMPLATE_UPDATE', `Template de relatório "${updatedTemplate.name}" atualizado.`, existingTemplate.municipality_id]
    )
...
router.delete('/templates/:id', requireSupervisor, async (req, res) => {
  try {
    ...
    await query(
      'INSERT INTO activity_logs (user_id, user_name, action, details, municipality_id) VALUES ($1, $2, $3, $4, $5)',
      [req.user.id, req.user.name, 'REPORT_TEMPLATE_DELETE', `Template de relatório "${existingTemplate.name}" excluído.`, existingTemplate.municipality_id]
    )
