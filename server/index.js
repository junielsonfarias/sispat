...
if (!process.env.JWT_SECRET) {
  ...
  console.error('💡 Solução: Verifique se o arquivo .env existe e contém JWT_SECRET=sua_chave_secreta')
  process.exit(1)
}

if (!process.env.DB_HOST) {
  console.error('🚨 ERRO CRÍTICO: Configurações do banco de dados não encontradas')
...
app.get('/debug/public-sync', async (req, res) => {
  try {
    ...
    const municipalities = await pool.query('SELECT id, name, state FROM municipalities')
    ...
    for (const municipality of municipalities.rows) {
      ...
      console.log(`📋 Município ${municipality.name}: ${patrimonios.rows.length} patrimônios`)
    }
...
app.post('/debug/force-public-sync', async (req, res) => {
  try {
    ...
    const municipalities = await pool.query('SELECT id, name, state FROM municipalities')
    console.log('📋 Municípios públicos encontrados:', municipalities.rows.length)
    ...
    `)
    console.log('📋 Patrimônios públicos encontrados:', patrimonios.rows.length)
...
app.post('/api/database/fix-patrimonios-schema', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      updatedColumns: [
        'cor', 'quantidade', 'forma_aquisicao', 'setor_responsavel', 
        'local_objeto', 'status', 'situacao_bem', 'fotos', 'documentos',
...
app.post('/api/database/create-missing-tables', async (req, res) => {
  try {
    ...
    const createMissingTables = (await import('./database/create-missing-tables.js')).default
    ...
    res.json({ 
      ...
      createdTables: [
        'transfers', 'form_fields', 'excel_csv_templates', 'customization_settings'
      ],
      updatedTables: [
        'imoveis', 'manutencao_tasks', 'inventories', 'report_templates', 'label_templates'
      ]
    })
  } catch (error) {
...
app.post('/api/database/create-sample-data', async (req, res) => {
  try {
    ...
    const createSampleData = (await import('./database/create-sample-data.js')).default
...
app.get('/api/debug/patrimonios-data', async (req, res) => {
  try {
    ...
    // Check total count
    const totalCount = await pool.query('SELECT COUNT(*) as total FROM patrimonios')
