...
const createIndex = async indexConfig => {
  ...
  try {
    ...
    if (!(await tableExists(indexConfig.table))) {
      logWarning(
        `Tabela ${indexConfig.table} não existe - pulando índice ${indexConfig.name}`
      );
  ...
  try {
    ...
    for (const idx of unusedIndexes) {
      try {
        ...
        removed.push(idx);
        logInfo(`Índice removido: ${idx.indexname}`, {
          table: idx.tablename,
          size: idx.size,
        });
      } catch (error) {
  ...
  try {
    ...
    for (const table of tables) {
      try {
        ...
        logError(
          `Erro ao fazer manutenção da tabela ${table.tablename}`,
          error
        );
      }
    }
