...
export const enablePerformanceMonitoring = async () => {
  try {
    ...
    if (extensions.length === 0) {
      logWarning(
        'Extensão pg_stat_statements não encontrada - tentando criar...'
      );
      try {
        ...
        logWarning(
          'Não foi possível criar pg_stat_statements - análise limitada',
          { error: error.message }
        );
        return false;
      }
    } else {
      logInfo('Extensão pg_stat_statements já está habilitada', {
...
export const analyzeSlowQueries = async () => {
  try {
    ...
    if (slowQueries.length > 0) {
      logWarning(`Encontradas ${slowQueries.length} queries lentas`, {
        ...
        slowQueries: slowQueries.map(q => ({
          ...
          hitPercent: q.hit_percent
            ? `${parseFloat(q.hit_percent).toFixed(2)}%`
            : 'N/A',
        })),
      });
    ...
    logWarning(
      'Não foi possível analisar queries lentas - pg_stat_statements pode não estar disponível',
      { error: error.message }
    );
    return [];
  }
...
export const analyzeIndexUsage = async () => {
  try {
    ...
    const unusedIndexes = indexUsage.filter(
      idx => idx.idx_scan === '0' || idx.idx_scan === 0
    );
...
export const analyzeTableStats = async () => {
  try {
    ...
    const needMaintenance = tableStats.filter(table => {
      const deadRatio =
        table.dead_tuples / (table.live_tuples + table.dead_tuples || 1);
...
export const suggestIndexes = async () => {
  try {
    ...
    const existingIndexNames = existingIndexes.map(idx =>
      idx.indexname.toLowerCase()
    );
