  ...
  try {
    ...
    for (const indexQuery of indexes) {
      try {
        ...
        createdCount++;
        console.log(
          `✅ Índice criado: ${indexQuery.split('idx_')[1].split(' ')[0]}`
        );
      } catch (error) {
        if (error.message.includes('already exists')) {
          skippedCount++;
          console.log(
            `⏭️ Índice já existe: ${indexQuery.split('idx_')[1].split(' ')[0]}`
          );
        } else {
    ...
    console.log(
      `✅ Otimização concluída: ${createdCount} índices criados, ${skippedCount} já existiam`
    );
...
export async function analyzeQueryPerformance() {
  try {
    ...
    const tablesWithoutIndexes = tableSizes.filter(
      table => !indexes.some(index => index.tablename === table.tablename)
    );
...
export async function optimizeSpecificQueries() {
  try {
    ...
    optimizations.push({
      ...
      optimization:
        'Índice composto criado para status + tipo + municipality_id',
      impact: 'high',
    });
...
export async function cleanupOldData() {
  try {
    ...
    const backupCount = await query(
      "SELECT COUNT(*) as count FROM pg_stat_user_tables WHERE tablename = 'backups'"
    );
...
export async function getPerformanceStats() {
  try {
    ...
    // Calcular hit ratio
    const hitRatio =
      stats.database.blks_hit && stats.database.blks_read
        ? (
            (stats.database.blks_hit /
              (stats.database.blks_hit + stats.database.blks_read)) *
            100
          ).toFixed(2)
        : 0;
