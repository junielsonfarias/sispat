...
export function validatePaginationParams(query) {
  ...
  try {
    validatedFilters = typeof filters === 'object' && filters !== null ? filters : {}
  } catch (error) {
...
export function buildPaginatedQuery(baseQuery, params, tableName = 't') {
  ...
  if (search) {
    ...
    searchFields.forEach(field => {
      if (typeof baseQuery === 'string' && baseQuery.toLowerCase().includes(field)) {
        searchConditions.push(`LOWER(${tableName}.${field}) LIKE LOWER($${paramIndex})`)
    ...
    if (searchConditions.length > 0) {
      ...
      query += typeof query === 'string' && query.toLowerCase().includes('where') 
        ? ` AND (${searchClause})`
        : ` WHERE (${searchClause})`
    }
  }
...
export const ENTITY_PAGINATION_CONFIG = {
  patrimonios: {
    ...
    filterFields: ['setor_responsavel', 'local_objeto', 'status', 'situacao_bem'],
    limit: 20
  },
  sectors: {
    defaultSort: 'name',
...
export function getEntityPaginationConfig(entityType) {
  return ENTITY_PAGINATION_CONFIG[entityType] || {
    defaultSort: 'created_at',
    defaultOrder: 'DESC',
    searchFields: ['name'],
    filterFields: [],
