...
- **Sprints 19, 20 e 21 concluídos (2026-05-12).** Ver `HISTORICO_CORRECOES.md`.
- **Sprint 19:** pacote `@sispat/shared`, middleware `zodValidate`, 6 rotas auth + 3 páginas frontend migradas. `STRONG_PASSWORD_REGEX` unificado.
- **Sprint 20:** user/sector/local/tipoBem — 20 endpoints. `common` (UUID/pagination). `validation.ts`: 1066→814.
- **Sprint 21:** inventario/transfer/manutencao/notification/customization/document/labelTemplate/formaAquisicao/emprestimo — ~30 endpoints. `validation.ts`: 814→383 (-64% acumulado). 50+ schemas em `@sispat/shared`. 151 testes backend.
- **Sprint 22 concluída (2026-06-24):** patrimonio e imovel migrados para `@sispat/shared` (`schemas/patrimonio.ts`, `schemas/imovel.ts`). Rotas `patrimonioRoutes`/`imovelRoutes` usam `zodValidate`; `patrimonioValidations`/`imovelValidations`/`queryValidations` removidos de `validation.ts`. `src/lib/validations/{patrimonio,imovel}Schema.ts` viraram reexports finos de `@sispat/shared` (nomes estáveis, zero mudança nos imports das páginas). Decisão de design do imóvel: front e back NÃO unificados num único schema (campos divergem: `area_total`≠`area_terreno`, `setor`≠`sectorId`, etc.) — `imovelFrontendSchema` valida o form e `createImovelBodySchema`/`updateImovelBodySchema` validam o que o backend lê. **Guard crítico:** schemas de body do backend usam `.passthrough()` porque o `zodValidate` substitui `req.body` e os services leem campos extras (`sectorId`/`localId`/`tipoId`/`acquisitionFormId`/`customFields`/`url_documentos`) — sem passthrough haveria perda silenciosa de dados. 10 testes novos em `__tests__/shared/patrimonioImovelSchemas.test.ts` travam passthrough + regras estritas do frontend (valor>0; kit≥2; endereço≥5). Verificado: shared build OK, backend tsc + 470 testes Jest, frontend type-check no baseline (zero erro líquido).
- **Sprint 22 (cont.):** rotas `/:id/notes` e `/:id/baixa` do patrimônio também migradas para `zodValidate` (`addNoteSchema`/`registrarBaixaSchema` em `@sispat/shared`). `data_baixa`/`motivo_baixa` ficam opcionais no schema de propósito (o controller mantém a checagem de obrigatoriedade com mensagem específica); `observacoes` declarado para não ser descartado. `patrimonioRoutes` não usa mais `express-validator`.
- **Sprint 23 concluída (2026-06-24):** `TipoBemManagement` e `AcquisitionFormManagement` (frontend) deixaram de definir schemas Zod locais e passaram a derivar de `createTipoBemSchema`/`createFormaAquisicaoSchema` do `@sispat/shared` (`.extend({ ativo })` para o campo de formulário; status real continua via endpoint de toggle). Elimina o drift: as regras do form agora batem com as que o backend valida (inclui o regex de nome do tipoBem, que faltava no front). Validação alinhada ao canônico (nome 1–100; descrição ≤500) sem regredir comportamento — confirmado por diff stash (frontend no baseline, zero erro líquido). **Item #13 (centralização Zod) concluído.**
