...
- **A11y:** ✅ `<TableHead>` sem `scope="col"` — resolvido **app-wide** adicionando `scope="col"` ao componente base `TableHead` (`ui/table.tsx`); ✅ `aria-label` adicionado a 16 ícone-botões (admin/superuser + listas de imóveis/transferências), além dos já feitos em #6/#7. Pendente: `loading="lazy"`/`alt` em imagens, drag-and-drop por teclado.
