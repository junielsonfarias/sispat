...
- **Comando único de dev (`npm run dev:all`):** `concurrently` sobe backend (nodemon :3000) + frontend (vite :8080) com prefixos `[backend]`/`[frontend]`; Ctrl+C para os dois. `.npmrc` `verify-deps-before-run=false` evita que o gate de build-scripts não aprovados (core-js/esbuild) aborte `pnpm run <script>` com `ERR_PNPM_IGNORED_BUILDS` (era a causa do `pnpm-workspace.yaml` ficar sujo com bloco `allowBuilds:` placeholder no sandbox).
