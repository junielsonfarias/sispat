...
async function checkSectors() {
  try {
    ...
    // Verificar setores
    const sectorsResult = await pool.query('SELECT id, name, municipality_id FROM sectors ORDER BY name');
    console.log(`\n📋 Total de setores encontrados: ${sectorsResult.rows.length}`);
    
    if (sectorsResult.rows.length > 0) {
      console.log('\n📝 Setores:');
      sectorsResult.rows.forEach((row, index) => {
        console.log(`${index + 1}. ${row.name} (ID: ${row.id}, Município: ${row.municipality_id})`);
      });
    } else {
