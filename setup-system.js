    ...
    try {
      ...
      if (health.status === 'OK') {
        ...
        await this.log(
          'Verificação de saúde',
          'error',
          'Servidor não está respondendo corretamente'
        );
        return false;
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.success) {
        await this.log(
          'Criação de backup',
          'success',
          `Backup criado: ${result.data.filename}`,
          result.data
        );
        return true;
      } else {
  ...
  async optimizePerformance() {
    try {
      await this.log(
        ...
        'info',
        'Otimizando performance do banco...'
      );
    ...
    try {
      ...
      if (result.success) {
        await this.log(
          'Listagem de backups',
          'success',
          `${result.data.length} backups encontrados`,
          result.data
        );
        return result.data;
      } else {
  ...
  async runFullSetup() {
    ...
    if (!serverHealthy) {
      await this.log(
        'Setup',
        'error',
        'Servidor não está disponível. Abortando setup.'
      );
      return this.results;
    }
    ...
    console.log(
      `📊 Resumo: ${this.results.summary.success}/${this.results.summary.total} etapas bem-sucedidas`
    );
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log(
          'Uso: node setup-system.js [full|quick|backup|optimize|stats]'
        );
