...
function initializeApp() {
  ...
  function onError(message: string, source?: string, line?: number, column?: number, error?: Error) {
    events.push([message, source, line, column, error])
    
    // Log do erro para desenvolvimento
    if (APP_CONFIG.development.enableDebugLogs) {
      console.error('🚨 SISPAT Error:', { message, source, line, column, error })
    }
  ...
  if (APP_CONFIG.development.enableDebugLogs) {
    console.log('🚀 SISPAT - Sistema de Gestão Patrimonial inicializado com sucesso!')
