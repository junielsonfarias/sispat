...
export const MapView: React.FC<MapViewProps> = ({ 
  ...
  const filterImoveis = useCallback(() => {
    const filtered = imoveis.filter(imovel => {
      const matchesSearch = searchTerm === '' || 
        imovel.numero_patrimonio.toLowerCase().includes(searchTerm.toLowerCase()) ||
        imovel.descricao.toLowerCase().includes(searchTerm.toLowerCase()) ||
        imovel.endereco.toLowerCase().includes(searchTerm.toLowerCase())
      
      const matchesTipo = selectedTipo === 'todos' || imovel.tipo === selectedTipo
      const matchesSetor = selectedSetor === 'todos' || imovel.setor === selectedSetor
  ...
  const addCustomControls = () => {
    ...
    `
    mapInstanceRef.current.controls[window.google.maps.ControlPosition.TOP_RIGHT].push(mapTypeControl)
    ...
    `
    mapInstanceRef.current.controls[window.google.maps.ControlPosition.TOP_RIGHT].push(locationControl)
  ...
  const handleImovelClick = (imovel: Imovel) => {
    ...
    if (imovel.latitude && imovel.longitude && mapInstanceRef.current) {
      mapInstanceRef.current.setCenter({ lat: imovel.latitude, lng: imovel.longitude })
  ...
  const tipos = ['todos', 'residencial', 'comercial', 'industrial', 'educacional', 'saude', 'religioso', 'armazem', 'predio']
  const setores = ['todos', ...Array.from(new Set(imoveis.map(i => i.setor).filter(Boolean)))]
      ...
      <Card className="mb-4">
        ...
        <CardContent>
          ...
          <div className="mt-4 flex gap-4 text-sm text-muted-foreground">
            ...
            <span>Com coordenadas: {imoveis.filter(i => i.latitude && i.longitude).length}</span>
          </div>
        </CardContent>
      </Card>
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute top-4 left-4 flex flex-col gap-2">
            <Button
              ...
              className={mapType === 'street' ? 'bg-primary text-primary-foreground' : ''}
            >
              Rua
            </Button>
            <Button
            ...
            <Button
              ...
              className={mapType === 'hybrid' ? 'bg-primary text-primary-foreground' : ''}
            >
              Híbrido
            </Button>
          </div>
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h4 className="font-medium text-sm">{imovel.numero_patrimonio}</h4>
                          <p className="text-xs text-muted-foreground mt-1">{imovel.descricao}</p>
                          <p className="text-xs text-muted-foreground mt-1">{imovel.endereco}</p>
                        </div>
