...
const MonitoringDashboard: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg">
      ...
      {/* Status Geral */}
      <div className="mb-6">
        <div className={`inline-flex items-center px-4 py-2 rounded-full ${getStatusColor(getHealthStatus())}`}>
          <div className={`w-3 h-3 rounded-full mr-2 ${
            getHealthStatus() === 'healthy' ? 'bg-green-500' :
            getHealthStatus() === 'warning' ? 'bg-yellow-500' : 'bg-red-500'
          }`}></div>
          Status: {getHealthStatus() === 'healthy' ? 'Saudável' : 
                   getHealthStatus() === 'warning' ? 'Atenção' : 'Crítico'}
        </div>
      </div>
      ...
      {metrics && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold text-green-900">{metrics.memory}%</p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-purple-600">Usuários Ativos</h3>
            <p className="text-2xl font-bold text-purple-900">{metrics.activeUsers}</p>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg">
            <h3 className="text-sm font-medium text-orange-600">Taxa de Erro</h3>
            <p className="text-2xl font-bold text-orange-900">{metrics.errorRate}%</p>
          </div>
        </div>
      )}
