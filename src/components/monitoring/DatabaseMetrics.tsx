...
const mockData: DatabaseMetricsData = {
  ...
  slowQueryLogs: [
    {
      id: '1',
      query: 'SELECT p.*, m.nome as municipio_nome FROM patrimonio p LEFT JOIN municipio m ON p.municipio_id = m.id WHERE p.status = \'ATIVO\' ORDER BY p.data_aquisicao DESC',
      duration: 2340,
    ...
    {
      id: '2',
      query: 'UPDATE patrimonio SET valor_atual = valor_atual * 1.05 WHERE categoria_id IN (SELECT id FROM categoria WHERE tipo = \'EQUIPAMENTO\')',
      duration: 1890,
    ...
    {
      id: '3',
      query: 'SELECT COUNT(*) as total, AVG(valor_atual) as valor_medio FROM patrimonio p JOIN municipio m ON p.municipio_id = m.id GROUP BY m.nome',
      duration: 1456,
  ...
  tableStats: [
    { table: 'patrimonio', queryCount: 456, avgDuration: 178, errorRate: 1.2, slowQueries: 12 },
    { table: 'municipio', queryCount: 234, avgDuration: 89, errorRate: 0.5, slowQueries: 2 },
    { table: 'categoria', queryCount: 189, avgDuration: 67, errorRate: 0.8, slowQueries: 1 },
    { table: 'usuario', queryCount: 167, avgDuration: 134, errorRate: 2.1, slowQueries: 5 },
    { table: 'log_atividade', queryCount: 123, avgDuration: 234, errorRate: 0.3, slowQueries: 8 }
  ],
  alerts: [
    {
      id: 'alert1',
      query: 'SELECT * FROM patrimonio WHERE status = ? ORDER BY data_aquisicao',
      duration: 2100,
      threshold: 1000,
...
export function DatabaseMetrics() {
  ...
  const refreshData = async () => {
    ...
    setData(prev => ({
      ...prev,
      averageQueryTime: Math.round(prev.averageQueryTime + (Math.random() - 0.5) * 20),
      activeConnections: Math.max(1, prev.activeConnections + Math.round((Math.random() - 0.5) * 3)),
  ...
  const operationData = data.slowQueryLogs.reduce((acc, log) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Métricas do Banco de Dados</h2>
          ...
          {data.alerts.map(alert => (
            <Alert key={alert.id} variant="destructive">
              ...
              <AlertDescription>
                <div className="space-y-2">
                  <div>
                    <strong>Query Lenta Detectada:</strong> {formatQuery(alert.query, 100)}
                  </div>
                  <div className="text-sm">
                    Duração: {formatDuration(alert.duration)} | 
                    Frequência: {alert.frequency}x | 
                    Tabela: {alert.table}
                  </div>
                  {alert.suggestions && (
                    <div className="text-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getStatusColor(data.averageQueryTime, { warning: 200, critical: 500 })}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getStatusColor(data.slowQueries, { warning: 5, critical: 10 })}`}>
              {data.slowQueries}
            </div>
            <p className="text-xs text-muted-foreground">
              Últimos 5 minutos
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Distribuição de queries lentas por tipo</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={operationData}>
                ...
                <Bar dataKey="avgDuration" fill="#82ca9d" name="Duração Média (ms)" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        <Badge variant={query.operation === 'SELECT' ? 'default' : 'secondary'}>
                          {query.operation}
                        </Badge>
                    ...
                    <div className="text-right">
                      <div className={`text-lg font-bold ${getStatusColor(query.duration, { warning: 1000, critical: 2000 })}`}>
                  ...
                  {selectedQuery?.id === query.id && (
                    <div className="mt-4 pt-4 border-t space-y-2">
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                          <strong>Linhas Afetadas:</strong> {query.rowsAffected || 'N/A'}
                        </div>
                      </div>
                      <div>
              ...
              <tbody>
                {data.tableStats.map(stat => (
                  <tr key={stat.table} className="border-b hover:bg-gray-50">
                    ...
                    <td className={`p-2 text-right ${getStatusColor(stat.avgDuration, { warning: 200, critical: 500 })}`}>
                      {formatDuration(stat.avgDuration)}
                    </td>
                    <td className={`p-2 text-right ${getStatusColor(stat.errorRate, { warning: 2, critical: 5 })}`}>
                      {stat.errorRate.toFixed(1)}%
                    </td>
                    <td className={`p-2 text-right ${getStatusColor(stat.slowQueries, { warning: 5, critical: 10 })}`}>
                      {stat.slowQueries}
                    </td>
                  </tr>
                ))}
