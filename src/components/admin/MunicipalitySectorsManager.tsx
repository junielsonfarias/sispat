  ...
  const handleCreateDefaultSectors = async () => {
    ...
    try {
      const response = await api.post(
        `/sectors/create-default/${municipalityId}`
      );
      ...
      toast({
        title: 'Erro',
        description:
          error.response?.data?.error || 'Erro ao criar setores padrão',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Card className='w-full'>
      <CardHeader>
        ...
        <CardDescription>
          Configure os setores padrão para este município. Estes setores serão
          criados como setores raiz (sem setor pai).
        </CardDescription>
      </CardHeader>
