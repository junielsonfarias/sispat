...
export default function UserManagementUnified({ 
  ...
  const manageableUsers = useMemo(() => {
    if (userType === 'superuser') {
      // Superuser vê todos os usuários exceto outros superusers
      return users.filter((u) => u.role !== 'superuser')
    } else {
  ...
  const defaultTitle = userType === 'superuser' 
    ? 'Gerenciamento de Usuários' 
    : 'Gerenciar Usuários'
  
  const defaultDescription = userType === 'superuser'
    ? `Gerencie todos os usuários do sistema de ${MUNICIPALITY_NAME}.`
    : 'Gerencie todos os usuários do seu município.'
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {userType === 'superuser' ? 'Todos os Usuários' : 'Usuários do Município'}
          </CardTitle>
          <CardDescription>
