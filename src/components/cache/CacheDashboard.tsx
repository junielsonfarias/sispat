...
import { useCacheInvalidation, useCacheMonitor, useCacheWarmup } from '@/hooks/useAdvancedCache';
import {
    Activity,
    Database,
    MemoryStick,
...
export function CacheDashboard() {
  const { stats, metrics, isMonitoring, startMonitoring, stopMonitoring } = useCacheMonitor();
  const { invalidateByTags, invalidateByPattern, clearAll, getStats: _getStats } = useCacheInvalidation();
  ...
  const getStatusIcon = (hitRate: number) => {
    if (hitRate >= 0.8) return <TrendingUp className="h-4 w-4 text-green-600" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Dashboard de Cache</h2>
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            onClick={isMonitoring ? stopMonitoring : () => startMonitoring(5000)}
            variant="outline"
            size="sm"
          >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats.total.hitRate > 0.8 ? 'Ótima' : stats.total.hitRate > 0.6 ? 'Boa' : 'Baixa'}
            </div>
            <p className="text-xs text-muted-foreground">
              Baseado no hit rate
            </p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Percentual de acertos no cache</CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={historicalData}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Line type="monotone" dataKey="hitRate" stroke="#8884d8" name="Total" strokeWidth={2} />
                    <Line type="monotone" dataKey="redisHitRate" stroke="#82ca9d" name="Redis" />
                    <Line type="monotone" dataKey="memoryHitRate" stroke="#ffc658" name="Memória" />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={historicalData}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Area type="monotone" dataKey="memoryUsage" stroke="#8884d8" fill="#8884d8" fillOpacity={0.3} />
                  </AreaChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
        ...
        <TabsContent value="metrics" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Chaves mais acessadas e suas métricas</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <div key={metric.key} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-medium">
                        {((metric.hits / (metric.hits + metric.misses)) * 100).toFixed(1)}%
                      </div>
                      <div className="text-xs text-muted-foreground">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Remover entradas específicas do cache</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex space-x-2">
                    ...
                    />
                    <Button onClick={handleInvalidateByPattern} variant="outline">
                      <Trash2 className="h-4 w-4 mr-2" />
                      Invalidar
                    </Button>
                  </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Pré-carregar dados frequentemente acessados</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {isWarming && (
                  <div className="space-y-2">
                ...
                <div className="text-xs text-muted-foreground">
                  O aquecimento carrega dados essenciais no cache para melhorar a performance
                </div>
              </CardContent>
            </Card>
          </div>
