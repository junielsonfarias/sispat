...
export const IntrusionDetectionDashboard: React.FC = () => {
  ...
  const handleResolveAlert = (alertId: string) => {
    intrusionDetectionService.resolveAlert(alertId, 'admin', 'Resolvido via dashboard')
    loadDashboardData()
  ...
  const handleMarkFalsePositive = (alertId: string) => {
    intrusionDetectionService.markFalsePositive(alertId, 'admin', 'Falso positivo via dashboard')
    loadDashboardData()
    ...
    <div className="space-y-6">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">
                {stats.alertsGenerated > 0 
                  ? Math.round(((stats.alertsGenerated - stats.falsePositives) / stats.alertsGenerated) * 100)
                  : 100}%
              </div>
              <p className="text-xs text-muted-foreground">
                Alertas verdadeiros
      ...
      {alerts.filter(alert => alert.severity === 'critical').length > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            <strong>Atenção:</strong> {alerts.filter(alert => alert.severity === 'critical').length} alerta(s) 
            crítico(s) detectado(s). Verifique imediatamente.
          </AlertDescription>
        </Alert>
      )}
                    ...
                    <div key={alert.id} className="border rounded-lg p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-mono">{alert.events[0]?.ipAddress}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="truncate">{alert.events[0]?.userAgent || 'N/A'}</p>
                        </div>
                      </div>
                      ...
                      {alert.status === 'active' && (
                        <div className="flex gap-2 pt-2">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleUnblockIP(alert.events[0]?.ipAddress)}
                          >
                ...
                <div className="space-y-3">
                  {stats.topThreats.map((threat, index) => (
                    <div key={threat.type} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <Progress 
                          value={(threat.count / Math.max(...stats.topThreats.map(t => t.count))) * 100} 
                          className="w-20"
                        />
                        <span className="text-sm font-medium w-8 text-right">{threat.count}</span>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {stats.topSources.map((source, index) => (
                    <div key={source.ip} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <Badge variant="outline">#{index + 1}</Badge>
                        <div>
                          <span className="font-mono font-medium">{source.ip}</span>
                          <p className="text-xs text-muted-foreground">
                            {intrusionDetectionService.isIPBlocked(source.ip) ? 'Bloqueado' : 'Ativo'}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Progress 
                          value={(source.count / Math.max(...stats.topSources.map(s => s.count))) * 100} 
                          className="w-20"
                        />
                        ...
                        <Button
                          ...
                          disabled={!intrusionDetectionService.isIPBlocked(source.ip)}
                        >
