...
import { SENSITIVE_FIELDS_CONFIG, encryptionService, encryptionUtils } from '@/services/encryption'
...
export const EncryptionSettings: React.FC = () => {
  ...
  const loadEncryptionStatus = async () => {
    ...
    try {
      ...
      const totalFields = Object.values(SENSITIVE_FIELDS_CONFIG)
  ...
  const getEncryptionPercentage = () => {
    ...
    return (encryptionStatus.encryptedFields / encryptionStatus.totalFields) * 100
  }
  ...
  const getDaysUntilRotation = () => {
    if (!encryptionStatus) return 0
    return Math.ceil((encryptionStatus.nextRotation.getTime() - Date.now()) / (24 * 60 * 60 * 1000))
  }
    ...
    <div className="space-y-6">
      ...
      {encryptionStatus && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {encryptionStatus.encryptedFields}/{encryptionStatus.totalFields}
              </div>
              <div className="flex items-center gap-2 mt-2">
                <Progress value={getEncryptionPercentage()} className="flex-1" />
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Força da Chave</CardTitle>
              <Key className="w-4 h-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className={`text-2xl font-bold ${getKeyStrengthColor(encryptionStatus.keyStrength)}`}>
                {encryptionStatus.keyStrength}%
              </div>
              <p className="text-xs text-muted-foreground">
                {encryptionStatus.keyStrength >= 90 ? 'Excelente' : 
                 encryptionStatus.keyStrength >= 70 ? 'Boa' : 'Necessita rotação'}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Última Rotação</CardTitle>
              <RefreshCw className="w-4 h-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {Math.ceil((Date.now() - encryptionStatus.lastRotation.getTime()) / (24 * 60 * 60 * 1000))}d
              </div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${getDaysUntilRotation() < 30 ? 'text-orange-600' : ''}`}>
      ...
      {encryptionStatus && getDaysUntilRotation() < 30 && (
        <Alert>
          ...
          <AlertDescription>
            <strong>Atenção:</strong> A rotação da chave de criptografia está próxima. 
            Recomendamos fazer a rotação em breve para manter a segurança dos dados.
          </AlertDescription>
        </Alert>
      )}
        ...
        <TabsContent value="fields" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Defina quais campos devem ser criptografados para cada tipo de entidade
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {config.fields.length} campo(s) • Rotação a cada {config.rotationInterval} dias
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(enabled) => handleFieldConfigChange(entityType, enabled)}
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {config.fields.map(field => (
                        <Badge key={field} variant="outline" className="text-xs">
                          {config.enabled ? <Lock className="w-3 h-3 mr-1" /> : <Unlock className="w-3 h-3 mr-1" />}
                          {field}
                        </Badge>
                      ))}
                    </div>
        ...
        <TabsContent value="keys" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Controle as chaves de criptografia e realize rotações de segurança
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 font-mono text-sm bg-muted p-2 rounded">
                    {showMasterKey ? '***MASTER_KEY_HIDDEN_FOR_SECURITY***' : '••••••••••••••••••••••••••••••••'}
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-4">
                  <Button
                    ...
                    variant={getDaysUntilRotation() < 30 ? 'default' : 'outline'}
                  >
                    {rotatingKey ? (
                ...
                <p className="text-xs text-muted-foreground">
                  A rotação de chave irá re-criptografar todos os dados sensíveis com uma nova chave.
              ...
              <div className="space-y-3">
                ...
                <Alert>
                  ...
                  <AlertDescription className="text-sm">
                    <strong>Importante:</strong> Mantenha backups seguros das chaves em local 
                    separado e criptografado. Sem as chaves, os dados não podem ser recuperados.
                  </AlertDescription>
                </Alert>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <Select defaultValue="AES-256">
                    ...
                    <SelectContent>
                      <SelectItem value="AES-256">AES-256-CBC (Recomendado)</SelectItem>
                ...
                <div className="space-y-3">
                  ...
                  <Select defaultValue="10000">
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="100000">100,000 (Máxima Segurança)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">Ativar rotação automática de chaves</span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch defaultChecked />
                    <span className="text-sm">Cache para melhor performance</span>
                  </div>
                </div>
              </div>
              ...
              <Alert>
                ...
                <AlertDescription>
                  <strong>Status:</strong> Todas as configurações estão otimizadas para 
                  máxima segurança e performance adequada.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
