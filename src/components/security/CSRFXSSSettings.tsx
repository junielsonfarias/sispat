...
import { useCSRFProtection, useSecurityProtection, useXSSProtection } from '@/hooks/useCSRFProtection'
import { csrfProtectionService, DEFAULT_CSRF_CONFIG } from '@/services/csrfProtection'
import { DEFAULT_XSS_CONFIG, xssProtectionService } from '@/services/xssProtection'
import {
    AlertTriangle,
    Bug,
    CheckCircle,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Origens Permitidas</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Tags Permitidas</CardTitle>
            <Bug className="w-4 h-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{xssConfig.allowedTags.length}</div>
            <p className="text-xs text-muted-foreground">
              Tags HTML seguras
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-sm">
              {csrf.lastRefresh ? csrf.lastRefresh.toLocaleTimeString() : 'Nunca'}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button variant="outline" size="sm" onClick={copyToken} disabled={!csrf.token}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCsrfConfig(prev => ({ ...prev, tokenName: e.target.value }))}
                  />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    onChange={(e) => setCsrfConfig(prev => ({ ...prev, headerName: e.target.value }))}
                  />
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      setCsrfConfig(prev => ({ ...prev, doubleSubmitCookie: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      setCsrfConfig(prev => ({ ...prev, originValidation: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      setCsrfConfig(prev => ({ ...prev, refererValidation: checked }))
                    }
                  />
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      setXssConfig(prev => ({ ...prev, sanitizeOnInput: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => 
                      setXssConfig(prev => ({ ...prev, sanitizeOnOutput: checked }))
                    }
                  />
                </div>
                  ...
                  {testResult.error ? (
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        ...
                        <Alert variant={testResult.xssDetection.detected ? 'destructive' : 'default'}>
                          <AlertDescription>
                            <div className="space-y-2">
                              <div className="flex items-center gap-2">
                                <Badge className={getRiskLevelColor(testResult.xssDetection.riskLevel)}>
                                  {testResult.xssDetection.riskLevel.toUpperCase()}
                                </Badge>
                                {testResult.xssDetection.detected ? (
                                  <span className="text-red-600">Ameaças detectadas</span>
                                ) : (
                                  <span className="text-green-600">Nenhuma ameaça detectada</span>
                                )}
                              </div>
                              {testResult.xssDetection.threats.length > 0 && (
                                <div>
                                  <strong>Ameaças:</strong> {testResult.xssDetection.threats.join(', ')}
                                </div>
                              )}
                            </div>
                          </AlertDescription>
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea
                          value={testResult.sanitized.sanitized || testResult.sanitized}
                          readOnly
                          rows={4}
                          className="font-mono text-sm"
                        />
