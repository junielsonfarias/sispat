...
import { httpSecurityService, DEFAULT_HTTPS_CONFIG, DEFAULT_CSP_DIRECTIVES } from '@/services/httpSecurity'
...
export const HTTPSSecuritySettings: React.FC = () => {
    ...
    try {
      ...
      if (valid) {
        toast({
          ...
          description: 'Todos os headers de segurança estão configurados corretamente',
        })
      } else {
        toast({
  ...
  const addCSPSource = (directive: keyof typeof cspDirectives, source: string) => {
  ...
  const removeCSPSource = (directive: keyof typeof cspDirectives, index: number) => {
    setCspDirectives(prev => ({
      ...prev,
      [directive]: Array.isArray(prev[directive])
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={testSecurityHeaders} disabled={testingHeaders}>
            {testingHeaders ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Contexto Seguro</CardTitle>
            <Shield className="w-4 h-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-2">
              <Badge variant={securityStatus.isSecureContext ? 'default' : 'destructive'}>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Headers Aplicados</CardTitle>
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getSecurityScore() >= 90 ? 'text-green-600' : 
                                                 getSecurityScore() >= 70 ? 'text-yellow-600' : 'text-red-600'}`}>
      ...
      {cspViolations.length > 0 && (
        <Alert>
          <AlertTriangle className="w-4 h-4" />
          <AlertDescription>
            <strong>Violações CSP detectadas:</strong> {cspViolations.length} violação(ões) 
            da Política de Segurança de Conteúdo foram registradas.
            <Button variant="link" size="sm" onClick={clearViolations} className="ml-2">
              Limpar
            </Button>
          </AlertDescription>
        </Alert>
      ...
      <Tabs defaultValue="https" className="space-y-4">
        ...
        <TabsContent value="https" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure a imposição de HTTPS e HSTS (HTTP Strict Transport Security)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  onChange={(e) => 
                    setHttpsConfig(prev => ({ ...prev, hstsMaxAge: parseInt(e.target.value) || 0 }))
                  }
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => 
                    setHttpsConfig(prev => ({ ...prev, hstsIncludeSubdomains: checked }))
                  }
                />
              </div>
        ...
        <TabsContent value="csp" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Configure as políticas de segurança de conteúdo para prevenir XSS e injeção de código
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                if (typeof value === 'boolean') {
                  return (
                    <div key={directive} className="flex items-center justify-between">
                      <div className="space-y-0.5">
                        <Label>{directive.replace(/([A-Z])/g, '-$1').toLowerCase()}</Label>
                      </div>
                      <Switch
                        checked={value}
                        onCheckedChange={(checked) => 
                          setCspDirectives(prev => ({ ...prev, [directive]: checked }))
                        }
                      />
                    </div>
                  )
                      ...
                      <div className="flex flex-wrap gap-2">
                        {value.map((source, index) => (
                          <Badge 
                            ...
                            onClick={() => removeCSPSource(directive as keyof typeof cspDirectives, index)}
                          >
                            {source} ×
                          </Badge>
                        ))}
                        ...
                        <Input
                          ...
                          onKeyPress={(e) => {
                            if (e.key === 'Enter') {
                              ...
                              addCSPSource(directive as keyof typeof cspDirectives, input.value)
                              input.value = ''
                            }
                          }}
                        />
        ...
        <TabsContent value="headers" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {Object.entries(httpSecurityService.generateSecurityHeaders()).map(([name, value]) => (
        ...
        <TabsContent value="violations" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Monitore violações CSP e outros problemas de segurança em tempo real
              </CardDescription>
            </CardHeader>
            <CardContent>
              {cspViolations.length === 0 ? (
                ...
                <div className="space-y-3">
                    ...
                    <Alert key={index}>
                      ...
                      <AlertDescription>
                        <div className="space-y-1">
                          <p><strong>{violation.type.toUpperCase()}:</strong> {violation.message}</p>
                          <p className="text-xs text-muted-foreground">
                            Fonte: {violation.source} • {violation.timestamp.toLocaleTimeString()}
                          </p>
                        </div>
                      </AlertDescription>
                    </Alert>
                  ...
                  {cspViolations.length > 10 && (
                    <p className="text-sm text-muted-foreground text-center">
                      Mostrando últimas 10 violações de {cspViolations.length} total
                    </p>
                  )}
                </div>
              )}
