...
export const Pagination = ({
  ...
  const handlePageChange = useCallback(
    async (cursor: string | null, direction: 'next' | 'prev' | 'first') => {
      setIsLoading(true);
      try {
  ...
  return (
    <div
      className={cn(
        'flex items-center justify-between space-x-6 py-4',
        className
      )}
    >
      ...
      <div className='flex items-center space-x-2'>
        ...
        <Button
          ...
          size='sm'
          onClick={() =>
            handlePageChange(extractCursorFromUrl(meta.prevUrl), 'prev')
          }
          disabled={!meta.hasPrev || isLoading}
          className='h-8 w-8 p-0'
        >
        ...
        <Button
          ...
          size='sm'
          onClick={() =>
            handlePageChange(extractCursorFromUrl(meta.nextUrl), 'next')
          }
...
export const usePagination = (
  ...
  const setPage = useCallback(
    (newCursor: string | null, direction?: 'next' | 'prev' | 'first') => {
      if (direction === 'first') {
        setCursor(null);
      } else {
        setCursor(newCursor);
  ...
  const setSort = useCallback(
    (newSort: string, newOrder: 'ASC' | 'DESC' = 'DESC') => {
