...
export const LabelPreview = forwardRef<HTMLDivElement, LabelPreviewProps>(
  ({ patrimonio, asset, template, className, onElementClick, selectedElementId }, ref) => {
    ...
    const finalAsset = asset || (patrimonio ? { ...patrimonio, assetType: 'bem' as const } : null)
    const settings = getSettingsForMunicipality(finalAsset?.municipalityId || null)
    ...
    const renderElement = (element: LabelElement) => {
      ...
      switch (element.type) {
        case 'LOGO':
          ...
          break
        case 'QR_CODE': {
          const publicUrl = finalAsset ? generatePublicQRUrl({
            id: finalAsset.id,
            assetType: finalAsset.assetType,
            numero_patrimonio: finalAsset.assetType === 'bem' ? (finalAsset as Patrimonio).numero_patrimonio : undefined
