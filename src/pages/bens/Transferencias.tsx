...
const Transferencias = () => {
  const { transferencias, updateTransferenciaStatus, createTransferencia } = useTransfers()
  const { user } = useAuth()
  ...
  const handleUpdateStatus = async (id: string, status: 'aprovada' | 'rejeitada') => {
    if (!user) return
    await updateTransferenciaStatus(id, status, { id: user.id, name: user.name })
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!user || !selectedPatrimonio || !setorOrigem || !setorDestino || !motivo) {
  ...
  const renderForm = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Preencha os dados para solicitar a transferência de um bem patrimonial.
        </CardDescription>
      </CardHeader>
      <CardContent>
