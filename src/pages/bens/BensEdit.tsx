...
const BensEdit = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      const formData = {
        ...
        data_aquisicao: data.data_aquisicao ? format(new Date(data.data_aquisicao), 'yyyy-MM-dd') : '',
        ...
        data_baixa: data.data_baixa ? format(new Date(data.data_baixa), 'yyyy-MM-dd') : '',
  ...
  useEffect(() => {
    if (user && user.role === 'usuario' && user.sectors && user.sectors.length === 1 && !selectedSectorName) {
      const primarySector = user.sectors.find(s => s.isPrimary) || user.sectors[0]
      console.log('🔍 Usuário normal com apenas um setor - selecionando automaticamente:', primarySector.name)
