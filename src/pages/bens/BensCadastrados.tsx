...
const BensCadastrados = () => {
  ...
  useEffect(() => {
    ...
    console.log('👤 Usuário:', user ? { id: user.id, name: user.name, role: user.role, municipalityId: user.municipalityId } : 'Não autenticado')
    console.log('🏠 BensCadastrados - Patrimônios recebidos:', patrimonios.length)
    console.log('📋 Patrimônios:', patrimonios.map(p => ({ 
      id: p.id, 
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold text-blue-600">{patrimonios.length}</p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow">
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold text-purple-600">{filteredPatrimonios.length}</p>
        </div>
      </div>
                ...
                <tr key={patrimonio.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                      patrimonio.status === 'ativo' ? 'bg-green-100 text-green-800' :
                      patrimonio.status === 'manutencao' ? 'bg-yellow-100 text-yellow-800' :
                      patrimonio.status === 'baixado' ? 'bg-red-100 text-red-800' :
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(endIndex, filteredPatrimonios.length)}</span> de{' '}
                  <span className="font-medium">{filteredPatrimonios.length}</span> resultados
                </p>
              </div>
              <div>
      ...
      {filteredPatrimonios.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">
            {searchTerm ? 'Nenhum patrimônio encontrado para sua busca.' : 'Nenhum patrimônio cadastrado.'}
          </p>
          {searchTerm && (
            <button
