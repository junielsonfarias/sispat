...
const AnaliseTemporal = () => {
  ...
  const timelineEvents = useMemo(() => {
    // Verificar se patrimonios existe e tem dados
    if (!patrimonios || patrimonios.length === 0) {
      return [];
    }

    const events = patrimonios.flatMap(p => {
      // Verificar se historico_movimentacao existe e é um array
      if (
        !p.historico_movimentacao ||
        !Array.isArray(p.historico_movimentacao)
      ) {
        return [];
      }
            ...
            <CardContent>
              {timelineEvents.length === 0 ? (
                <div className='text-center py-8 text-muted-foreground'>
                  <p>Nenhum evento encontrado no histórico de movimentação.</p>
                  <p className='text-sm'>
                    Os eventos aparecerão aqui conforme as movimentações forem
                    registradas.
                  </p>
                </div>
              ) : (
