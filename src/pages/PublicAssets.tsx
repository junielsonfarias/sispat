...
export default function PublicAssets() {
  ...
  const publicMunicipalities = useMemo(
    () => {
      ...
      console.log('  - Municipalities:', municipalities.length, municipalities.map(m => ({ id: m.id, name: m.name })))
      console.log('  - PublicSettings:', publicSettings)
      console.log('  - PublicMunicipalityIds:', publicSettings.publicMunicipalityIds)
      
      const filtered = municipalities.filter((m) =>
        publicSettings.publicMunicipalityIds.includes(m.id),
      )
      
      console.log('  - Filtered public municipalities:', filtered.length, filtered.map(m => ({ id: m.id, name: m.name })))
      ...
      if (filtered.length === 0 && municipalities.length > 0) {
        console.log('⚠️ Nenhum município público válido encontrado, incluindo todos os municípios disponíveis')
  ...
  useEffect(() => {
    const fetchPublicData = async () => {
      ...
      try {
        ...
        const bensRes = await fetch(`/api/patrimonios/public?municipalityId=${selectedMunicipalityId}&limit=500`)
        ...
        const bens: CombinedAsset[] = Array.isArray(bensJson)
          ? bensJson.map((p: any) => ({ 
              ...
              data_aquisicao: p.data_aquisicao || new Date().toISOString().split('T')[0],
              fotos: p.fotos || [],
              // Garantir que municipalityId está presente
              municipalityId: p.municipality_id || p.municipalityId || selectedMunicipalityId
            }))
          : []

        // Buscar imóveis públicos
        const imoveisRes = await fetch(`/api/imoveis/public?municipalityId=${selectedMunicipalityId}&limit=500`)
        const imoveisJson = imoveisRes.ok ? await imoveisRes.json() : []
        const imoveisList: CombinedAsset[] = Array.isArray(imoveisJson)
          ? imoveisJson.map((i: any) => ({ ...i, assetType: 'imovel' as const }))
          : []

        const merged = [...bens, ...imoveisList]
        setPublicAssets(merged)
        console.log('🌐 Public data loaded:', { bens: bens.length, imoveis: imoveisList.length })
  ...
  const processedData = useMemo(() => {
    ...
    const filtered = combinedData.filter((p) => {
      ...
      const municipalityMatch = publicAssets.length > 0 ? true : p.municipalityId === selectedMunicipalityId
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-800 mb-2">Consulta Pública de Bens</h1>
        ...
        {!selectedMunicipalityId ? (
          <Card className="w-full max-w-md mx-auto mt-10 animate-fade-in shadow-xl border-0 bg-white/90 backdrop-blur-sm">
            <CardHeader className="text-center">
              <CardTitle className="text-2xl font-bold text-gray-800">Selecione um Município</CardTitle>
              ...
              {paginatedData.length > 0 ? (
                        ...
                        {paginatedData.map((item) => (
                          <tr key={item.id} className="hover:bg-blue-50 transition-colors duration-200">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                              {item.assetType === 'bem'
                                ? (item as Patrimonio).descricao || (item as any).descricao || 'Sem descrição'
                                : (item as Imovel).denominacao || 'Sem denominação'}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                              {(item as Patrimonio).setor_responsavel || 'Não informado'}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                              {(item as Patrimonio).local_objeto || 'Não informado'}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                {(item as Patrimonio).situacao_bem || 'Não informado'}
                              </span>
                            </td>
                ...
                <div className="col-span-full text-center py-16">
                  <div className="bg-white rounded-xl shadow-lg p-12 border border-gray-100">
                    ...
                    <h3 className="text-2xl font-semibold mb-4 text-gray-800">Nenhum bem encontrado</h3>
            ...
            <div className="mt-8">
              <Pagination
                meta={{
                  ...
                  nextUrl: pagination.pageIndex < pageCount ? `?page=${pagination.pageIndex + 1}` : undefined,
                  prevUrl: pagination.pageIndex > 1 ? `?page=${pagination.pageIndex - 1}` : undefined
                }}
