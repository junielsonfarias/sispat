...
export const SecuritySettings: React.FC<SecuritySettingsProps> = () => {
  ...
  const terminateAllSessions = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/terminate-sessions`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Último login: {new Date(securityStats.lastLogin).toLocaleString('pt-BR')}
                  </p>
                </div>
                <Button 
                  variant="outline" 
              ...
              <Alert>
                <Shield className="h-4 w-4" />
                <AlertDescription>
                  Encerrar todas as sessões irá desconectar você de todos os dispositivos. 
