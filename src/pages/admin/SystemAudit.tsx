...
import { Loader2, CheckCircle, XCircle, AlertTriangle, Info, Database, Globe, Settings, Users, Building2, MapPin, Package, Navigation, Zap } from 'lucide-react'
...
export default function SystemAudit() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: hasErrors ? 'Alguns testes falharam' : 'Todos os testes passaram',
  ...
  const runNavigationTests = async () => {
    ...
    try {
      ...
      toast({
        title: hasErrors ? 'Testes de Navegação com Falhas' : 'Testes de Navegação Concluídos',
  ...
  const runCompleteSystemTest = async () => {
    ...
    try {
      ...
      toast({
        title: hasErrors ? 'Teste Completo com Falhas' : 'Teste Completo Concluído',
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Verificação completa de todas as funcionalidades e integridade dos dados
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={runTests} disabled={isTesting} variant="outline">
            {isTesting ? (
              ...
              <>
                ...
                Testes de Criação
                     ...
                     <Button onClick={runCRUDTests} disabled={isTesting} variant="secondary">
             {isTesting ? (
               <>
                 <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                 Testando CRUD...
           ...
           <Button onClick={runNavigationTests} disabled={isTesting} variant="outline">
             {isTesting ? (
               <>
                 <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                 Testando Navegação...
           ...
           <Button onClick={runCompleteSystemTest} disabled={isTesting} variant="secondary">
             {isTesting ? (
               <>
                 <Loader2 className="mr-2 h-4 w-4 animate-spin" />
      ...
      <Tabs defaultValue="audit" className="space-y-4">
           ...
           <TabsTrigger value="navigation-tests">Testes de Navegação</TabsTrigger>
           <TabsTrigger value="complete-test">Teste Completo do Sistema</TabsTrigger>
         </TabsList>
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Executada em {new Date(auditResult.timestamp).toLocaleString('pt-BR')}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Status: {getStatusBadge(auditResult.summary.system_status)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>Recomendações: {auditResult.summary.total_recommendations}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Object.entries(auditResult.database.tables).map(([tableName, tableInfo]: [string, any]) => (
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(auditResult.api_endpoints).map(([endpoint, info]: [string, any]) => (
                      <div key={endpoint} className="border rounded-lg p-4">
                        ...
                        <div className="text-sm text-muted-foreground">
                          ...
                          {info.sample_data && (
                            <p>Dados de exemplo: {info.sample_data.length} registros</p>
                          )}
                        </div>
                      </div>
                    ))}
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    {auditResult.errors.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-red-600 mb-2">Erros ({auditResult.errors.length})</h4>
                    ...
                    {auditResult.warnings.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-yellow-600 mb-2">Avisos ({auditResult.warnings.length})</h4>
                        <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="border rounded-lg p-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {auditResult.database.data_integrity.users?.length || 0} usuários
                      </p>
                    </div>
                    ...
                    <div className="border rounded-lg p-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {auditResult.database.data_integrity.sectors?.length || 0} setores
                      </p>
                    </div>
                    ...
                    <div className="border rounded-lg p-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {auditResult.database.data_integrity.locals?.length || 0} locais
                      </p>
                    </div>
                    ...
                    <div className="border rounded-lg p-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {auditResult.database.data_integrity.patrimonios?.length || 0} patrimônios
                      </p>
                    </div>
                  </div>
                </CardContent>
          ...
          {testResult && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Executados em {new Date(testResult.timestamp).toLocaleString('pt-BR')}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {Object.entries(testResult.tests).map(([testName, testInfo]: [string, any]) => (
              ...
              <CardContent>
                {testResult.summary && (
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-2">
                      <AlertTriangle className="h-4 w-4 text-yellow-500" />
                      <span>Erros: {testResult.summary.errors?.length || 0}</span>
                    </div>
                  </div>
                )}
                
                <div className="space-y-4">
                  {Object.entries(testResult.tests).map(([moduleName, moduleTests]: [string, any]) => (
                    <div key={moduleName} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant={moduleTests.failed === 0 ? "default" : "destructive"}>
                            {moduleTests.passed}/{moduleTests.total_operations}
                          </Badge>
                        </div>
                      </div>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                        {Object.entries(moduleTests.operations || {}).map(([operation, result]: [string, any]) => (
                      ...
                      {moduleTests.errors && moduleTests.errors.length > 0 && (
                        <div className="mt-4 p-3 bg-red-50 rounded-lg">
                          <h5 className="font-medium text-red-800 mb-2">Erros:</h5>
                          <div className="space-y-1">
                            {moduleTests.errors.map((error: string, index: number) => (
                ...
                {testResult.summary?.errors && testResult.summary.errors.length > 0 && (
                  <div className="mt-6 p-4 bg-red-50 rounded-lg">
                    <h4 className="font-semibold text-red-800 mb-3">Erros Gerais:</h4>
                    <div className="space-y-2">
                      {testResult.summary.errors.map((error: string, index: number) => (
                        <Alert key={index} variant="destructive">
                          <XCircle className="h-4 w-4" />
                          <AlertDescription>{error}</AlertDescription>
                        </Alert>
           ...
           {testResult && testResult.tests && (
             <Card>
               <CardHeader>
                 ...
                 <CardDescription>
                   Testes de funcionalidades do menu lateral e navegação entre páginas
                 </CardDescription>
               </CardHeader>
               <CardContent>
                 {testResult.summary && (
                     ...
                     <div key={moduleName} className="border rounded-lg p-4">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                         {Object.entries(moduleTests.operations || {}).map(([operation, result]: [string, any]) => (
                           <div key={operation} className="border rounded p-3">
                             <div className="flex items-center justify-between mb-2">
                               <span className="font-medium capitalize">{operation.replace(/_/g, ' ')}</span>
                               {getStatusIcon(result.status)}
                             </div>
                             {result.status === 'SUCCESS' ? (
                               <div className="text-sm text-green-600">
           ...
           {completeTestResult && (
             <Card>
               <CardHeader>
                 ...
                 <CardDescription>
                   Simulação completa de uso do sistema - Duração: {Math.round(completeTestResult.duration / 1000)}s
                 </CardDescription>
               </CardHeader>
               <CardContent>
                 <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                   <div className="flex items-center gap-2">
                     <Info className="h-4 w-4 text-blue-500" />
                     <span>Total: {completeTestResult.summary.total_operations}</span>
                   </div>
                   <div className="flex items-center gap-2">
                     <CheckCircle className="h-4 w-4 text-green-500" />
                   ...
                   <div className="flex items-center gap-2">
                     <AlertTriangle className="h-4 w-4 text-yellow-500" />
                     <span>Erros Críticos: {completeTestResult.summary.critical_errors}</span>
                   </div>
                   <div className="flex items-center gap-2">
                     <Database className="h-4 w-4 text-purple-500" />
                     <span>Status: {getStatusBadge(completeTestResult.summary.system_status)}</span>
                   </div>
                 </div>
                 
                 <div className="space-y-4">
                   {Object.entries(completeTestResult.tests).map(([testName, testResult]: [string, any]) => (
                     <div key={testName} className="border rounded-lg p-4">
                       <div className="flex items-center justify-between mb-4">
                         <h4 className="font-semibold text-lg capitalize">
                           {testName.replace(/_/g, ' ')}
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                         {Object.entries(testResult.operations || {}).map(([operation, result]: [string, any]) => (
                           <div key={operation} className="border rounded p-3">
                             <div className="flex items-center justify-between mb-2">
                               <span className="font-medium capitalize">{operation.replace(/_/g, ' ')}</span>
                               {getStatusIcon(result.status)}
                 ...
                 {completeTestResult.errors && completeTestResult.errors.length > 0 && (
                   <div className="mt-6 p-4 bg-red-50 rounded-lg">
                     <h4 className="font-semibold text-red-800 mb-3">Erros Críticos:</h4>
                     <div className="space-y-2">
                       {completeTestResult.errors.map((error: string, index: number) => (
                         <Alert key={index} variant="destructive">
                           <XCircle className="h-4 w-4" />
                           <AlertDescription>{error}</AlertDescription>
                         </Alert>
                 ...
                 {completeTestResult.warnings && completeTestResult.warnings.length > 0 && (
                   <div className="mt-6 p-4 bg-yellow-50 rounded-lg">
                     <h4 className="font-semibold text-yellow-800 mb-3">Avisos:</h4>
                     <div className="space-y-2">
                       {completeTestResult.warnings.map((warning: string, index: number) => (
