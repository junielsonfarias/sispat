...
export default async function handler(
  ...
  try {
    ...
    const [serverInfo, memoryInfo, statsInfo, persistenceInfo] =
    ...
    const healthStatus: RedisHealthStatus = {
      ...
      redis: {
        ...
        responseTime,
        version:
          serverInfo.status === 'fulfilled' ? serverInfo.value.version : null,
      ...
      persistence:
        persistenceInfo.status === 'fulfilled'
          ? persistenceInfo.value
          : {
              lastSave: null,
    ...
    if (
      healthStatus.memory.fragmentation > 2.0 ||
