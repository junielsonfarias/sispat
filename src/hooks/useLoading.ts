...
export const useListLoading = <T>(options: UseListLoadingOptions = {}): UseListLoadingReturn<T> => {
  ...
  const withListLoading = useCallback(async <U>(
    asyncFn: (page: number, pageSize: number) => Promise<{
      items: U[]
      total: number
      hasMore: boolean
    ...
    try {
      ...
      const errorMsg = errorMessage || (err instanceof Error ? err.message : 'Erro ao carregar dados')
      setError(errorMsg)
    } finally {
      setLoading(false)
    }
