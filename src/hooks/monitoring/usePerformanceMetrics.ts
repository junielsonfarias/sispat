...
import { APIMetrics, DatabaseMetrics, SystemMetrics } from '../../services/monitoring/performanceMetrics';
...
export function usePerformanceMetrics(
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    const delay = opts.reconnectInterval * Math.pow(2, Math.min(reconnectAttempts.current - 1, 3));

    console.log(`Tentando reconectar em ${delay}ms (tentativa ${reconnectAttempts.current}/${opts.maxReconnectAttempts})`);
  ...
  const handleMessage = useCallback((message: any) => {
    switch (message.type) {
      ...
      case 'metric':
        if (message.data) {
          setData(prevData => ({
            api: message.data.api || prevData?.api || {
              requestsPerSecond: 0,
              averageResponseTime: 0,
              errorRate: 0,
              activeRequests: 0,
            ...
            database: message.data.database || prevData?.database || {
              averageQueryTime: 0,
              slowQueries: 0,
              activeConnections: 0,
              totalQueries: 0
