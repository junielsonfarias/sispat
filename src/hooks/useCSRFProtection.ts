...
import { csrfProtectionService, csrfUtils, CSRFToken } from '@/services/csrfProtection'
import { xssProtectionService, SanitizationResult } from '@/services/xssProtection'
...
export const useCSRFProtection = () => {
  ...
  const generateToken = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Erro ao gerar token CSRF'
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage
  ...
  const validateToken = useCallback((token: string, method: string = 'POST') => {
  ...
  const addCSRFHeaders = useCallback((headers: Record<string, string> = {}) => {
    if (!state.token) {
  ...
  const createProtectedForm = useCallback((formElement: HTMLFormElement) => {
    if (!state.token) {
      throw new Error('Token CSRF não disponível')
    }
...
export const useXSSProtection = () => {
  // Sanitiza HTML
  const sanitizeHTML = useCallback((html: string, customConfig?: any): SanitizationResult => {
  ...
  const sanitizeProps = useCallback((props: Record<string, any>) => {
    const sanitized = { ...props }
    
    Object.keys(sanitized).forEach(key => {
      if (typeof sanitized[key] === 'string') {
        if (key.toLowerCase().includes('html') || key === 'dangerouslySetInnerHTML') {
...
export const useSecurityProtection = () => {
  ...
  const secureRequest = useCallback(async (
    ...
    const headers = csrf.addCSRFHeaders(options.headers as Record<string, string> || {})
    ...
    if (typeof body === 'string') {
      ...
      if (xssDetection.detected && xssDetection.riskLevel === 'high') {
        throw new Error(`Conteúdo bloqueado: ${xssDetection.threats.join(', ')}`)
      }
  ...
  const validateInput = useCallback((input: string, type: 'text' | 'html' = 'text') => {
    // Detecta XSS
    const xssDetection = xss.detectXSS(input)
    
    if (xssDetection.detected) {
      if (xssDetection.riskLevel === 'high') {
        throw new Error(`Input bloqueado: ${xssDetection.threats.join(', ')}`)
      }
    ...
    if (type === 'html') {
      ...
      return { sanitized: xss.sanitizeText(input), safe: !xssDetection.detected }
    }
  }, [xss])
