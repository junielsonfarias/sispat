...
import { TwoFactorSetup, generateTwoFactorSecret, verifyTwoFactorToken, verifyBackupCode } from '@/services/twoFactorAuth'
...
const TwoFactorContext = createContext<TwoFactorContextData>({} as TwoFactorContextData)
...
export const TwoFactorProvider: React.FC<TwoFactorProviderProps> = ({ children }) => {
  ...
  const completeSetup = useCallback(async (verificationToken: string): Promise<boolean> => {
    ...
    try {
      ...
      const isValid = verifyTwoFactorToken(verificationToken, setupData.secret)
  ...
  const verifyBackup = useCallback(async (code: string): Promise<boolean> => {
    if (!user?.id) {
      return false
    }
  ...
  const regenerateBackupCodes = useCallback(async (): Promise<string[]> => {
    ...
    try {
      ...
      toast({
        title: 'Sucesso',
        description: 'Novos códigos de backup gerados. Guarde-os em local seguro.',
      })
