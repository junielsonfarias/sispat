...
export const LoadingProvider = ({ children }: LoadingProviderProps) => {
  ...
  const setGlobalLoading = useCallback(
    (loading: boolean, message = 'Carregando...') => {
  ...
  const addLoadingState = useCallback(
    (key: string, message: string, progress?: number) => {
      setLoadingStates(prev => {
        const existing = prev.find(state => state.key === key);
        if (existing) {
          return prev.map(state =>
            state.key === key ? { ...state, message, progress } : state
          );
        }
...
export const useGlobalLoading = () => {
  ...
  if (!context) {
    throw new Error(
      'useGlobalLoading deve ser usado dentro de um LoadingProvider'
    );
  }
  return context;
};
...
export const useComponentLoading = (componentKey: string) => {
  const { addLoadingState, updateLoadingState, removeLoadingState } =
    useGlobalLoading();
