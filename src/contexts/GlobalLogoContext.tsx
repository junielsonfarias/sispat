...
interface GlobalLogoContextType {
  ...
  getLogoForSystem: (systemType: 'etiqueta' | 'ficha' | 'relatorio' | 'geral') => string
}

const GlobalLogoContext = createContext<GlobalLogoContextType | null>(null)

const defaultLogoUrl = 'https://img.usecurling.com/i?q=brazilian%20government&color=azure'

export const GlobalLogoProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [globalLogoUrl, setGlobalLogoUrlState] = useState<string>(defaultLogoUrl)
  const { user } = useAuth()
  ...
  const getLogoForSystem = (systemType: 'etiqueta' | 'ficha' | 'relatorio' | 'geral') => {
...
export const useGlobalLogo = () => {
  ...
  if (!context) {
    throw new Error('useGlobalLogo deve ser usado dentro de um GlobalLogoProvider')
  }
  return context
}
