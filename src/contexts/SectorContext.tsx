...
export const SectorProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchSectors = useCallback(async (municipalityId?: string) => {
    if (!user) {
      setIsLoading(false)
      return
    }
  ...
  const fetchSectorsByMunicipality = useCallback(async (municipalityId: string) => {
    ...
    if (!user) {
      ...
      try {
        setIsLoading(true)
        const data = await api.get<Sector[]>(`/sectors/public/${municipalityId}`)
        console.log('✅ Setores carregados (público):', data?.length || 0, 'setores')
        setSectors(data || [])
      } catch (error) {
