...
export const PublicSearchProvider = ({ children }: { children: ReactNode }) => {
    ...
    if (stored) {
      ...
      if (cachedMunicipalities) {
        try {
          ...
          console.log('✅ PublicSearchProvider - Creating new settings:', updatedSettings)
          ...
          console.error('❌ Erro ao criar configurações a partir dos municípios:', error)
          localStorage.setItem(
            'sispat_public_search_settings',
        ...
        console.log('⚠️ PublicSearchProvider - No cached municipalities, using initial settings')
        localStorage.setItem(
          'sispat_public_search_settings',
          JSON.stringify(initialSettings),
        )
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      const cachedMunicipalities = localStorage.getItem('sispat_municipalities')
      const currentSettings = localStorage.getItem('sispat_public_search_settings')
      ...
      if (cachedMunicipalities && currentSettings) {
        try {
          ...
          const missingIds = allMunicipalityIds.filter(id => !settings.publicMunicipalityIds.includes(id))
          if (missingIds.length > 0) {
            const updatedSettings = {
              ...settings,
              publicMunicipalityIds: [...new Set([...settings.publicMunicipalityIds, ...allMunicipalityIds])]
            }
            console.log('🔄 PublicSearchProvider - Updating settings with new municipalities:', updatedSettings)
            localStorage.setItem('sispat_public_search_settings', JSON.stringify(updatedSettings))
            setSettings(updatedSettings)
          }
        } catch (error) {
