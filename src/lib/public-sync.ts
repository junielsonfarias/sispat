...
export function checkPublicDataSync(): PublicSyncStatus {
  ...
  if (publicSettings) {
    try {
      ...
      status.publicSettings = parsed.isPublicSearchEnabled && Array.isArray(parsed.publicMunicipalityIds)
    } catch {
  ...
  try {
    ...
    if (syncResponse.ok) {
      ...
      if (syncData.municipalities && Array.isArray(syncData.municipalities)) {
        localStorage.setItem('sispat_municipalities', JSON.stringify(syncData.municipalities))
        console.log('✅ Municípios sincronizados:', syncData.municipalities.length)
      }
      
      // Salvar configurações públicas
      if (syncData.publicSettings) {
        localStorage.setItem('sispat_public_search_settings', JSON.stringify(syncData.publicSettings))
        console.log('✅ Configurações públicas atualizadas')
      }
      
      // Salvar patrimônios
      if (syncData.patrimonios && Array.isArray(syncData.patrimonios)) {
        localStorage.setItem('sispat_patrimonios', JSON.stringify(syncData.patrimonios))
        console.log('✅ Patrimônios sincronizados:', syncData.patrimonios.length)
      }
      
      // Salvar dados agrupados por município
      if (syncData.patrimoniosByMunicipality) {
        localStorage.setItem('sispat_patrimonios_by_municipality', JSON.stringify(syncData.patrimoniosByMunicipality))
      ...
      console.error('❌ Erro na resposta da sincronização:', syncResponse.status, syncResponse.statusText)
  ...
  try {
    ...
    if (municipalitiesResponse.ok) {
      ...
      if (Array.isArray(municipalities)) {
        localStorage.setItem('sispat_municipalities', JSON.stringify(municipalities))
        console.log('✅ Municípios sincronizados (legado):', municipalities.length)
        ...
        localStorage.setItem('sispat_public_search_settings', JSON.stringify(publicSettings))
    ...
    for (const municipalityId of allMunicipalityIds) {
      try {
        const patrimoniosResponse = await fetch(`/api/patrimonios/public?municipalityId=${municipalityId}&limit=100`)
        if (patrimoniosResponse.ok) {
          ...
          if (Array.isArray(patrimonios) && patrimonios.length > 0) {
            ...
            const existing = JSON.parse(localStorage.getItem('sispat_patrimonios') || '[]')
            const merged = [...existing, ...patrimonios]
            const unique = merged.filter((item, index, arr) => arr.findIndex(i => i.id === item.id) === index)
            localStorage.setItem('sispat_patrimonios', JSON.stringify(unique))
            console.log(`✅ Patrimônios públicos sincronizados para ${municipalityId} (legado):`, patrimonios.length)
          }
        }
      } catch (error) {
        console.log(`ℹ️ Patrimônios públicos não disponíveis para município ${municipalityId}`)
      }
    }
 ...
 */
export function generatePublicQRUrl(asset: { id: string; assetType: 'bem' | 'imovel'; numero_patrimonio?: string }): string {
