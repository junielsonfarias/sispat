...
export const PatrimonioSchema = z.object({
  ...
  data_aquisicao: z.date().max(new Date(), 'Data não pode ser futura').optional(),
  ...
  quantidade: z.number().int().positive('Quantidade deve ser um número positivo').default(1),
  fotos: z.array(z.string()).optional(),
  documentos: z.array(z.string()).optional(),
  metodo_depreciacao: z.string().optional(),
  vida_util_anos: z.number().int().positive('Vida útil deve ser um número positivo').optional(),
  valor_residual: z.number().positive('Valor residual deve ser positivo').optional(),
  status: z.enum(['ativo', 'manutencao', 'inativo', 'baixado']).default('ativo')
})
...
export function safeValidate<T>(schema: z.ZodSchema<T>, data: unknown): Partial<T> {
  try {
...
export function validateBase64Image(base64String: string, maxSizeMB = 5): { valid: boolean; error?: string } {
  try {
    ...
    if (sizeInBytes > maxSizeBytes) {
      return { valid: false, error: `Imagem muito grande. Máximo permitido: ${maxSizeMB}MB` }
    }
    ...
    if (!mimeType || !allowedTypes.includes(mimeType)) {
      return { valid: false, error: 'Tipo de imagem não permitido. Use JPEG, PNG, WebP ou GIF' }
    }
