...
class HTTPSecurityService {
  ...
  constructor(
    ...
    csp: CSPDirectives = DEFAULT_CSP_DIRECTIVES
  ) {
    this.config = config;
  ...
  addNonceToCSP(nonce: string): void {
    this.cspDirectives['script-src'] = [
      ...this.cspDirectives['script-src'].filter(
        src => !src.startsWith("'nonce-")
      ),
      `'nonce-${nonce}'`,
    ];
  }
  ...
  validateSecurityConfig(): { valid: boolean; warnings: string[] } {
    ...
    if (this.config.hstsMaxAge < 31536000) {
      warnings.push(
        'HSTS max-age deveria ser pelo menos 1 ano (31536000 segundos)'
      );
    }
    ...
    if (
      !this.cspDirectives['frame-ancestors'].includes("'none'") &&
