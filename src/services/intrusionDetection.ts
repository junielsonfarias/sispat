...
export const DEFAULT_INTRUSION_RULES: IntrusionRule[] = [
  ...
  },
  {
    ...
    pattern: /(\bunion\b|\bselect\b|\binsert\b|\bupdate\b|\bdelete\b|\bdrop\b|\bor\s+1\s*=\s*1|\band\s+1\s*=\s*1)/i,
    severity: 'critical',
    threshold: 1,
    timeWindow: 60000,
    enabled: true,
  ...
  },
  {
    ...
    pattern: /(<script|javascript:|on\w+\s*=|<iframe|<object|eval\s*\(|alert\s*\()/i,
    severity: 'high',
    threshold: 1,
    timeWindow: 60000,
    enabled: true,
  ...
  },
  {
    ...
    pattern: /(sqlmap|nikto|nmap|masscan|zap|burp|wget|curl.*bot|python-requests)/i,
    severity: 'medium',
    threshold: 1,
    timeWindow: 60000,
    enabled: true,
...
class IntrusionDetectionService {
   ...
   */
  async recordEvent(eventData: Omit<IntrusionEvent, 'id' | 'timestamp'>): Promise<void> {
  ...
  getDetectionStats(): DetectionStats {
    ...
    return {
      ...
      alertsGenerated: this.alerts.filter(a => a.timestamp.getTime() > last24h).length,
      blockedIPs: Array.from(this.blockedIPs.values()).filter(exp => exp > now).length,
      blockedUsers: Array.from(this.blockedUsers.values()).filter(exp => exp > now).length,
      falsePositives: this.alerts.filter(a => 
        a.status === 'false_positive' && 
        a.timestamp.getTime() > last24h
      ).length,
      topThreats,
      topSources
    }
  ...
  private async analyzeEvent(event: IntrusionEvent): Promise<void> {
    ...
    for (const rule of this.rules.values()) {
      ...
      if (rule.pattern instanceof RegExp) {
        ...
        matches = rule.pattern.test(payload) || 
  ...
  private eventMatchesRule(event: IntrusionEvent, rule: IntrusionRule): boolean {
    if (rule.pattern instanceof RegExp) {
      const payload = JSON.stringify(event.payload || '')
  ...
  private async triggerAlert(rule: IntrusionRule, events: IntrusionEvent[]): Promise<void> {
  ...
  private async executeAction(
    ...
    switch (action.type) {
      ...
      case 'log':
        console.log(`📋 LOG DE SEGURANÇA: ${alert.rule.name} - IP: ${triggerEvent.ipAddress}`)
        break
