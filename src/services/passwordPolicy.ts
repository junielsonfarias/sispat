...
const COMMON_PASSWORDS = [
  ...
  'iloveyou', 'adobe123', '123321', 'admin', 'password123', 'solo',
  'monkey', 'lovely', 'shadow', 'princess', 'dragon', 'passw0rd',
  'master', 'hello', 'freedom', 'whatever', 'qazwsx', 'trustno1',
  'jordan23', 'harley', 'robert', 'matthew', 'jordan', 'asshole',
  'daniel', 'andrew', 'martin', 'welcome', 'michael', 'charlie',
  'maggie', 'superman', '696969', 'qwertyuiop', 'hottie', 'batman',
  'loveme', 'zaq1zaq1', 'password2', 'secretpassword', 'reddog',
  'love', '112233', 'starwars', 'klaster', 'zxcvbnm', 'hunter',
  'thomas', 'michelle', 'tigger', 'prodigy', 'chocolate', 'baseball',
  'garfield', 'cheese', 'stewart', 'orange', 'hahaha', 'rainbow',
  'computer', 'michelle1', 'liverpool', 'princess1', 'jordan1',
  'michelle', 'snoopy', 'chicken', 'hockey', 'angels', 'madison',
  'bailey', 'junior', 'poohbear', 'lovers', 'stupid', 'startrek',
  'captain', 'coffee', 'scooter', 'buster', 'brown', 'biteme',
  'green', 'test', 'lakers', 'forever', 'batman1', 'trustme',
  'soccer', 'monster', 'dexter', 'morgan', 'batman', 'turtle'
]
...
export const validatePassword = (
  ...
  if (hasConsecutiveChars(password, policy.maxConsecutiveChars)) {
    errors.push(`A senha não pode ter mais de ${policy.maxConsecutiveChars} caracteres consecutivos iguais`)
  }
  ...
  if (previousPasswords && previousPasswords.length > 0) {
    const isReused = checkPasswordReuse(password, previousPasswords, policy.preventReuse)
    if (isReused) {
      errors.push(`Não é possível reutilizar uma das últimas ${policy.preventReuse} senhas`)
    }
  }
...
export const createPasswordSchema = (
  ...
  return z.string()
    .min(policy.minLength, `A senha deve ter pelo menos ${policy.minLength} caracteres`)
    .max(policy.maxLength, `A senha deve ter no máximo ${policy.maxLength} caracteres`)
    .refine((password) => {
      const validation = validatePassword(password, policy, userInfo, previousPasswords)
...
function checkUserInfoInPassword(password: string, userInfo: UserInfo): string[] {
  const errors: string[] = []
  
  if (containsUserInfo(password, userInfo)) {
    errors.push('A senha não pode conter informações pessoais (nome, email, etc.)')
  }
