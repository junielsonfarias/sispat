...
class EncryptionService {
   ...
   */
  decryptFromTransit(encryptedPayload: string, maxAge: number = 300000): any { // 5 min default
    try {
      ...
      throw new Error(`Erro ao descriptografar payload: ${error instanceof Error ? error.message : 'Erro desconhecido'}`)
    }
  }
  ...
  // Métodos privados
  private deriveKey(salt: CryptoJS.lib.WordArray, contextKey?: string, iterations?: number): CryptoJS.lib.WordArray {
    ...
    if (cached && Date.now() - cached.createdAt < 3600000) { // Cache por 1 hora
  ...
  private startCacheCleanup(): void {
    setInterval(() => {
      ...
      for (const [key, value] of this.keyCache.entries()) {
        if (now - value.createdAt > 3600000) { // 1 hora
...
export const encryptionUtils = {
   ...
   */
  maskSensitiveData(value: string, type: 'cpf' | 'cnpj' | 'phone' | 'email' | 'generic' = 'generic'): string {
    if (!value) return ''
    ...
    switch (type) {
      ...
      case 'email': {
        ...
        const maskedUser = user.length > 3 ? user.substring(0, 3) + '*'.repeat(user.length - 3) : user
