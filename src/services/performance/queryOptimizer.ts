  ...
  private performQueryAnalysis(
    ...
    if (upperQuery.startsWith('SELECT')) {
      ...
      if (!upperQuery.includes('LIMIT') && !upperQuery.includes('COUNT(')) {
        ...
        improvements.push(
          'Adicionado LIMIT 1000 para evitar retorno excessivo de dados'
        );
      ...
      if (upperQuery.includes('ORDER BY') && !upperQuery.includes('INDEX')) {
        improvements.push(
          'Considerar adicionar índices para campos de ordenação'
        );
    ...
    if (upperQuery.startsWith('INSERT')) {
      ...
      if (valuesCount > 1) {
        improvements.push(
          'Considerar usar INSERT em lote para múltiplos registros'
        );
    ...
    if (upperQuery.startsWith('UPDATE')) {
      if (!upperQuery.includes('WHERE')) {
        improvements.push(
          '⚠️ ATENÇÃO: UPDATE sem WHERE pode afetar todos os registros'
        );
    ...
    if (upperQuery.startsWith('DELETE')) {
      if (!upperQuery.includes('WHERE')) {
        improvements.push(
          '⚠️ ATENÇÃO: DELETE sem WHERE pode remover todos os registros'
        );
        estimatedTimeReduction += 50;
      }
    }
  ...
  private generateRecommendations(
    ...
    if (slowQueries.length > 0) {
      recommendations.push(
        `Identificadas ${slowQueries.length} queries lentas (>${this.slowQueryThreshold}ms)`
      );
    }
    ...
    if (mostAccessedTable && mostAccessedTable.avgTime > 500) {
      recommendations.push(
        `Tabela ${mostAccessedTable.table} tem tempo médio alto (${mostAccessedTable.avgTime.toFixed(2)}ms)`
      );
    }
    ...
    if (cacheUsage < 30) {
      recommendations.push(
        'Baixo uso de cache, considerar implementar mais cache'
      );
    }

    // Verificar operações de escrita
    const writeOperations = metrics.filter(
      m => m.operation !== 'SELECT'
    ).length;
    if (writeOperations > totalQueries * 0.3) {
      recommendations.push(
        'Alto volume de operações de escrita, considerar otimizações'
      );
    }
