...
class XSSProtectionService {
   ...
   */
  sanitizeHTML(html: string, customConfig?: Partial<XSSProtectionConfig>): SanitizationResult {
  ...
  validateURL(url: string): { valid: boolean; reason?: string } {
    ...
    try {
      ...
      if (url.includes('<script') || url.includes('javascript:') || url.includes('data:text/html')) {
   ...
   */
  sanitizeAttributes(attributes: Record<string, string>): Record<string, string> {
   ...
   */
  detectXSS(input: string): { detected: boolean; threats: string[]; riskLevel: 'low' | 'medium' | 'high' } {
    ...
    const xssPatterns = [
      ...
      { pattern: /javascript:/i, threat: 'JavaScript URL', risk: 'high' as const },
      { pattern: /on\w+\s*=/i, threat: 'Event handler', risk: 'medium' as const },
      ...
      { pattern: /data:text\/html/i, threat: 'Data URL HTML', risk: 'high' as const },
      { pattern: /vbscript:/i, threat: 'VBScript URL', risk: 'high' as const },
      { pattern: /expression\s*\(/i, threat: 'CSS expression', risk: 'medium' as const },
      { pattern: /@import/i, threat: 'CSS import', risk: 'low' as const },
      { pattern: /document\.cookie/i, threat: 'Cookie access', risk: 'high' as const },
      { pattern: /document\.write/i, threat: 'Document write', risk: 'medium' as const },
      { pattern: /eval\s*\(/i, threat: 'Eval function', risk: 'high' as const },
      { pattern: /alert\s*\(/i, threat: 'Alert function', risk: 'low' as const }
    ]
