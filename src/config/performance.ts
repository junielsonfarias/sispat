...
class PerformanceMonitor {
  ...
  getPerformanceStats() {
    const recentMetrics = this.metrics.filter(
      m => Date.now() - m.timestamp < 5 * 60 * 1000 // Últimos 5 minutos
    );

    const apiStats = this.apiMetrics.filter(
    ...
    return {
      ...
      averageLoadTime: this.calculateAverage(
        recentMetrics.filter(m => m.name === 'TotalLoadTime')
      ),
      averageAPITime: this.calculateAverage(
        apiStats.map(m => ({ value: m.duration }))
      ),
      apiSuccessRate:
        apiStats.length > 0
          ? (apiStats.filter(m => m.success).length / apiStats.length) * 100
          : 0,
...
export function usePerformanceMonitoring() {
  return {
    ...
    recordAPIMetric:
      performanceMonitor.recordAPIMetric.bind(performanceMonitor),
