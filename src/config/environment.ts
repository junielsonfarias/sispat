...
export function getEnvironmentConfig(): EnvironmentConfig {
  ...
  return {
    ...
    database: {
      ...
      name: import.meta.env.VITE_DB_NAME || (isProduction ? 'sispat_production' : 'sispat_development'),
      user: import.meta.env.VITE_DB_USER || 'sispat_user',
      password: import.meta.env.VITE_DB_PASSWORD || 'CHANGE_ME_STRONG_DB_PASSWORD_2025',
      ssl: isProduction,
      maxConnections: parseInt(import.meta.env.VITE_DB_MAX_CONNECTIONS || '100'),
    ...
    redis: {
      ...
      password: import.meta.env.VITE_REDIS_PASSWORD || 'CHANGE_ME_STRONG_REDIS_PASSWORD_2025',
      url: import.meta.env.VITE_REDIS_URL || `redis://:${import.meta.env.VITE_REDIS_PASSWORD || 'CHANGE_ME_STRONG_REDIS_PASSWORD_2025'}@localhost:6379`,
      retryDelayOnFailover: parseInt(import.meta.env.VITE_REDIS_RETRY_DELAY || '100'),
      maxRetriesPerRequest: parseInt(import.meta.env.VITE_REDIS_MAX_RETRIES || '3'),
    },
    
    jwt: {
      secret: import.meta.env.VITE_JWT_SECRET || 'CHANGE_ME_STRONG_JWT_SECRET_2025_MIN_32_CHARS_LONG',
      expiresIn: import.meta.env.VITE_JWT_EXPIRES_IN || '24h',
      refreshExpiresIn: import.meta.env.VITE_JWT_REFRESH_EXPIRES_IN || '7d',
    },
    
    cors: {
      origin: (import.meta.env.VITE_CORS_ORIGIN || 'http://localhost:3000,http://localhost:8080').split(','),
      credentials: import.meta.env.VITE_CORS_CREDENTIALS === 'true',
    },
    
    logging: {
      level: import.meta.env.VITE_LOG_LEVEL || (isProduction ? 'warn' : 'debug'),
      file: import.meta.env.VITE_LOG_FILE || (isProduction ? '/var/log/sispat/app.log' : './logs/app.log'),
    ...
    upload: {
      ...
      path: import.meta.env.VITE_UPLOAD_PATH || (isProduction ? '/var/sispat/uploads' : './uploads'),
      allowedTypes: (import.meta.env.VITE_UPLOAD_ALLOWED_TYPES || 'image/jpeg,image/png,image/gif,application/pdf').split(','),
      maxFiles: parseInt(import.meta.env.VITE_UPLOAD_MAX_FILES || '10'),
    },
    
    backup: {
      path: import.meta.env.VITE_BACKUP_PATH || (isProduction ? '/var/sispat/backups' : './backups'),
    ...
    monitoring: {
      ...
      metricsInterval: parseInt(import.meta.env.VITE_METRICS_INTERVAL || '30000'),
